...
4. Register it in `agent/agent.go` `buildSessionTools()` — always-on built-ins (Bash/Read/Write/Edit/Fetch, and `SessionInfo` — a small, purely informational session snapshot that needs no gating beyond `DisallowedTools`) just check `isToolAllowed`; an *optional* built-in that needs its own on/off switch (e.g. `WebSearch`) adds a new `AgentOptions.EnableX bool` field instead, gating registration with `if a.opts.EnableX && a.isToolAllowed(...)`. **Every new `AgentOptions.EnableX` field must also get a matching `harness.AgentWithX()` option in the root `harness.go` facade** (see `AgentWithMCPs`/`AgentWithMemory`/`AgentWithWebSearch` for the one-line pattern) — `EnableWebSearch` shipped without one for a full release cycle before an SDK user reported being unable to reach it from outside the CLI (`internal/cli/agent.go`'s `newInteractiveAgent` could set the struct field directly, but an external embedder using the public facade had no way to). Add a `TestAgentWithBoolOptionsSetTheirFlag` case (`harness_test.go`, root package) alongside it. (`SessionInfo` used to share a gate — `EnableSessionInfo` — with `SessionSearch`, a full-text search tool over the session's own history; `SessionSearch` was removed entirely for being largely unused in practice and redundant with persistent memory, at which point `SessionInfo` lost its flag too and became always-on — see `docs/plans/2026-09-22-sessionsearch-removal.md`.)
   ...
   **`*Session`'s read-side contract, after a deliberate audit-and-consolidate pass**: every public getter (`Model()`, `Thinking()`, `Stats()`, `MaxIterations()`, `Meta()`, plus the always-immutable `ID()`/`CWD()`/`CreatedAt()`/`Name()`) is LOCK-FREE — safe to call from literally anywhere, including from inside a tool executor (the tool executor runs inside a turn while `promptSync` holds `s.mu` for the whole turn, so anything that DID take `s.mu` there would deadlock instantly with no timeout) or an HTTP handler that might be racing a running turn (`server.go`'s `GET /api/sessions/{id}/info` and `GET /api/sessions/{id}` are built on exactly this — found and fixed live as a real blocking bug before this consolidation: a request against a busy session used to queue behind `s.mu` for as long as the turn took, 9.9s-60s+ observed, an actual regression against an endpoint documented as a "fast, read-only snapshot"). There is NO separate, differently-named public method that takes `s.mu` for a "more thorough" read — that distinction used to exist (`Meta()`/`Stats()` blocking vs. `CurrentModel()`/`CurrentThinking()`/`CurrentStats()`/`CurrentMaxIterations()`/`CurrentMeta()` lock-free) and was a genuine source of confusion for SDK consumers (which one do I call, and why are there two ways to get "the current stats"?). The blocking, drain-and-persist internals (folding `Subagent`/`Fetch` delegated cost — see `addDelegatedCost`'s doc comment — into `s.stats` and persisting it) are now the UNEXPORTED `syncMeta()`/`syncStats()`, used only by `agent.go` itself right before discarding an ephemeral sub-agent session; an external caller has no way to trigger the drain these exist for in the first place (`pendingDelegatedCost` is only ever fed by harness's own built-in tools), so there was never a reason to expose them.
