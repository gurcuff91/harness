...
4. Register it in `agent/agent.go` `buildSessionTools()` — always-on built-ins (Bash/Read/Write/Edit/Fetch) just check `isToolAllowed`; an *optional* built-in that needs its own on/off switch (e.g. `WebSearch`, `SessionInfo`/`SessionSearch`) adds a new `AgentOptions.EnableX bool` field instead, gating registration with `if a.opts.EnableX && a.isToolAllowed(...)`. **Every new `AgentOptions.EnableX` field must also get a matching `harness.AgentWithX()` option in the root `harness.go` facade** (see `AgentWithMCPs`/`AgentWithMemory`/`AgentWithWebSearch`/`AgentWithSessionInfo` for the one-line pattern) — `EnableWebSearch`/`EnableSessionInfo` shipped without one for a full release cycle before an SDK user reported being unable to reach them from outside the CLI (`internal/cli/agent.go`'s `newInteractiveAgent` could set the struct field directly, but an external embedder using the public facade had no way to). Add a `TestAgentWithBoolOptionsSetTheirFlag` case (`harness_test.go`, root package) alongside it.
