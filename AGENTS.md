...
4. Register it in `agent/agent.go` `buildSessionTools()` — always-on built-ins (Bash/Read/Write/Edit/Fetch) just check `isToolAllowed`; an *optional* built-in that needs its own on/off switch (e.g. `WebSearch`, `SessionInfo`/`SessionSearch`) adds a new `AgentOptions.EnableX bool` field instead, gating registration with `if a.opts.EnableX && a.isToolAllowed(...)`. **Every new `AgentOptions.EnableX` field must also get a matching `harness.AgentWithX()` option in the root `harness.go` facade** (see `AgentWithMCPs`/`AgentWithMemory`/`AgentWithWebSearch`/`AgentWithSessionInfo` for the one-line pattern) — `EnableWebSearch`/`EnableSessionInfo` shipped without one for a full release cycle before an SDK user reported being unable to reach them from outside the CLI (`internal/cli/agent.go`'s `newInteractiveAgent` could set the struct field directly, but an external embedder using the public facade had no way to). Add a `TestAgentWithBoolOptionsSetTheirFlag` case (`harness_test.go`, root package) alongside it. A tool needing a live view of session state (current model, thinking level, accumulated stats, full history) must read it through a LOCK-FREE getter/method on `*Session` (`CurrentModel()`, `CurrentThinking()`, `CurrentStats()`, `AllMessages()`, `SearchMessages()`, …) — never `Session.Meta()`/`Session.Stats()` or anything that takes `s.mu`: the tool executor runs inside a turn while `promptSync` already holds that lock for the whole turn, so taking it again deadlocks instantly with no timeout (see `agent/session_info_test.go`'s `TestSessionInfoGettersDoNotDeadlockUnderPromptSyncLock`/`TestSessionSearchMessagesDoesNotDeadlockUnderPromptSyncLock` and the `CurrentModel()` precedent they mirror). `CurrentStats()` mirrors `CurrentModel()`'s exact `atomic.Value` pattern — see it for how to add a new lock-free field: write the snapshot under `s.mu` at every point the guarded field mutates (including `Reset()` — a real bug where `Reset()` cleared the store's persisted stats but forgot the in-memory copy was found and fixed here, see `TestResetClearsInMemoryStats`), read it back with no lock at all. A tool that needs actual PERSISTENCE logic (not just a live read) belongs on the `SessionStore` interface itself (`agent/store/store.go`), not inside the tool — see `SearchMessages` for the pattern: the interface method, a thin `*Session`/`*store.Session` wrapper, and the tool reduced to an adapter that only parses/validates input and delegates to an injected func. `agent/tools` importing `agent/store` for shared result/error types (`store.SearchResult`, `store.ErrSearchNotSupported`) is fine — no import cycle (`agent/store` has zero dependency on `agent/tools`) — same as `client/types.go` already does for `store.SessionMeta`. This isn't only a tool-executor concern: any caller that wants to read live session state from a session that MIGHT be mid-turn — notably `server.go`'s own HTTP handlers for an already-active session (`GET /api/sessions/{id}/info`, `GET /api/sessions/{id}`) — has the exact same deadlock-shaped risk, just blocking instead of deadlocking outright (an HTTP handler isn't itself running inside `promptSync`'s lock, so it doesn't deadlock — it just queues behind `s.mu` for however long the turn takes, which is just as real a bug for an endpoint documented as a "fast, read-only snapshot"; found and fixed live — see `Session.CurrentMeta()`'s doc comment and `server/session_info_blocking_test.go`'s acceptance test, which measures a real in-flight turn and asserts the endpoint returns in the same order of magnitude as any other non-blocking read).
