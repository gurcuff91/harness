...
// only internal/cli constructs internal/logx.HarnessLogger{} and passes it
// in via WithLogger).
...
// logx.NilLogger{} — never this one — to the transport's own in-process
...
func TestWithLoggerSetsOptionsLogger(t *testing.T) {
	...
	o.logger.Info("slack", "test_event")
	if len(rec.lines) != 1 || rec.lines[0] != "slack test_event" {
...
// is responsible for defaulting a nil logger to logx.NilLogger{}, exactly
// like every other runner in this codebase (server.Run, telegram.Run).
func TestOptionsLoggerIsNilBeforeRunAppliesItsDefault(t *testing.T) {
	var o Options
	for _, opt := range []Option{WithWorkspace("x")} { // no WithLogger
		opt(&o)
	}
	if o.logger != nil {
