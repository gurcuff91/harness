...
// direct regression test for a real SDK-user report: AgentWithWebSearch
// didn't exist at all (EnableWebSearch was reachable only via
// AgentWithOptions or harness's own internal newInteractiveAgent, never as
// a standalone facade option like every other EnableX flag already had).
// Exercises AgentWithOptions + AgentOption application directly (not
// agent.New(), which would require a live provider) — this only needs to
// confirm the OPTION mutates the struct.
