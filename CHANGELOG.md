...
The emergency prune previously calibrated its character budget from `currentChars / lastInputTokens`. That ratio could compare image-stripped history against a token count from a different request, provider, model, or image mix, producing an unsafe budget and allowing an oversized compaction request through.

- The compaction budget now uses a fixed `2.0 chars/token` ratio at 95% of the active model context window. For a 1M-token window this yields a 1.9M-character budget, which catches dense code/JSON histories such as the reported `overbooking-app` session before compaction itself exceeds the provider limit.
- The prune remains oldest-turn-first and atomic: tool calls stay paired with their results, the newest turn is preserved, and omitted messages remain intact in the append-only JSONL history. Only the summary input is reduced.
- Added deterministic tests proving the fixed ratio is independent of stale token measurements and that the field-study-shaped history is pruned safely.

## [0.76.96] - 2026-09-22

### Fix — `/model` switch to a smaller-window provider could leave the session over its new context threshold undetected, and compacted with the wrong (too-small) model
Root-caused a real `anthropic API error 400: "prompt is too long: 1183771 tokens > 1000000 maximum"` reported live, traced end-to-end through `SwitchModel`. Two compounding gaps in the SAME function, both fixed together:

- **Stale `ContextUsage` across a model switch.** `SwitchModel` updates `s.contextWindow` to the new model's window, but `s.stats.ContextUsage` — the value the ReAct loop's auto-compact check reads (`s.stats.ContextUsage >= autoCompactThreshold`) — was only ever refreshed by `updateStats` (after a real turn completes) or `compactWithTarget` (after a real compaction). Switching from a bigger-window model (e.g. `minimax-cm-dev`, 1,048,576 tokens) to a smaller one (e.g. `claude-oauth`, 1,000,000) left `ContextUsage` computed against the OLD, larger window until the first turn on the new model already ran — so a session already close to the smaller model's own 95% threshold sailed past it with nothing to catch it until the provider itself rejected the request. Fix: `SwitchModel` now recomputes what `ContextUsage` WOULD be against the destination's window using the current `s.lastInputTokens`, and compacts preemptively if that crosses `autoCompactThreshold` — a new trigger alongside the pre-existing mandatory one (destination window smaller than the raw token count).
- **Compacting with the wrong model when the destination is smaller.** The existing "compact before switch" mechanism (commit `8c09f06`, undocumented at the time — a collateral change inside an unrelated "Codex context window metadata" commit) always compacted using the DESTINATION model. That made sense for the case it was built for — an origin provider gone stale/exhausted (expired token, rate-limited) used to get a session permanently stuck on `/model`, since the mandatory compact ran against the already-broken origin and failed every time. But it was never conditioned on the destination actually being large enough to hold the compaction itself: asking a SMALLER destination to summarize a history that may not even fit its own window risks producing the exact "prompt is too long" 400 this mechanism exists to prevent — confirmed live as the root cause of the reported error. Fix: `SwitchModel` now compacts using whichever of origin/destination has the LARGER context window, falling back to the destination only when it's strictly larger (preserving the original stuck-session fix). Deliberately no automatic fallback if the chosen (larger) model turns out to be exhausted — the provider's real error (401/429/expired-token) propagates straight out of `SwitchModel`, exactly as it did before the destination-compaction mechanism existed; silently retrying against a smaller destination would risk overflowing the exact same way and would mask that the origin needs reconnecting.
- New tests in `agent/switch_model_compact_test.go` covering all four combinations (destination larger → compacts with destination; destination smaller → compacts with origin; preventive trigger fires before the mandatory one would; no fallback when the chosen model is exhausted) against fake providers with controlled context windows — fully deterministic, no live model call needed. Reproduced live: temporarily reverted both fixes and confirmed 3 of the 5 new tests fail exactly as expected; restored and confirmed all green.
- **Scope note**: this closes the gap where a stale context-usage reading let a switch proceed unchecked. A SEPARATE, still-open gap (deliberately out of scope for this fix, per discussion) is that a single ReAct iteration can still grow past the auto-compact threshold entirely WITHIN itself — e.g. several large images pasted into the current, in-progress turn — since the top-of-loop check only re-evaluates usage between iterations, using the token count from the PREVIOUS provider response. That's a distinct, harder problem (accurately estimating in-flight image cost without a real API round-trip) left for future work.
...
Auditing the previous release's fix surfaced real confusion in `*Session`'s public surface: `Meta()`/`Stats()` (blocking, take `s.mu`) and `CurrentModel()`/`CurrentThinking()`/`CurrentStats()`/`CurrentMaxIterations()`/`CurrentMeta()` (lock-free) were two parallel ways to read overlapping information, with no name-level signal for which to use or why two existed. Consolidated into a single, lock-free-by-default contract.

- **`Model()`/`Thinking()`/`Stats()`/`MaxIterations()`/`Meta()`** are now the ONLY public read accessors — every one of them lock-free, safe to call from anywhere (a tool executor mid-turn, an HTTP handler racing a running turn). `CurrentModel()`→`Model()`, `CurrentThinking()`→`Thinking()`, `CurrentStats()`→`Stats()`, `CurrentMaxIterations()`/the old `MaxIterations()` wrapper→a single `MaxIterations()`, `CurrentMeta()`→`Meta()`. Pure renames — no behavior change to any of these five.
- **The old blocking `Meta()`/`Stats()`** (which took `s.mu` to drain+persist pending delegated `Subagent`/`Fetch` cost before reading) are now the UNEXPORTED `syncMeta()`/`syncStats()` — used only internally by `agent.go`, right before discarding an ephemeral sub-agent session. There was never an external way to trigger the drain these exist for (delegated cost is only ever fed by harness's own built-in tools), so this isn't a capability loss for SDK consumers — `Meta()`/`Stats()` already carry the same "may lag by up to one turn behind undrained delegated cost" staleness window any concurrent reader always had.
- `server/server.go`'s three call sites (`handleSessionInfo`, `handleGetSession`, `handleResumeSession`) now uniformly call `Meta()`/`MaxIterations()` — no more distinguishing which of two methods to use depending on whether the session might be mid-turn.
- Tests renamed/re-audited to match: `TestModelReflectsSwitchModel`, `TestModelDoesNotDeadlockUnderPromptSyncLock`, `TestModelConcurrentReadsWhileSwitchModelWrites`, `TestMetaDoesNotBlockUnderPromptSyncLock`, `TestMaxIterationsReflectsSetMaxIterations`. Two tests needed a real semantic fix (not just a rename) since they depended on the OLD blocking `Stats()`'s drain-on-read behavior with no turn ever running to trigger it naturally: `TestAddDelegatedCostIsInvisibleToContextUsage` and the renamed `TestAddDelegatedCostDrainedBySyncMeta` now call `syncStats()`/`syncMeta()` explicitly — reproduced live that using the new lock-free `Stats()` in their place makes the first test fail (confirmed the exact silent-breakage risk this migration could have introduced), then restored the correct call.
- Removed `TestSubagentReadonlyOverrideReachesEphemeralSubAgent` (`agent/subagent_readonly_test.go`): a live-provider integration test whose pass/fail depended on the model faithfully self-reporting whether it had a Write/Edit tool — confirmed flaky in practice (one real run failed because the model's own response text claimed a file was created despite `readonly=true` correctly stripping Write/Edit from its tool registry, a model-honesty artifact, not a harness bug). The actual mechanical guarantee (does `readonly:true` reach the executor, foreground and background) is already covered deterministically against a mock executor by `agent/tools/subagent_test.go`'s `TestSubagentReadonlyTrueReachesExecutor`/`TestSubagentReadonlyTrueReachesExecutorInBackgroundMode` — this integration test added no coverage those don't already provide, only flakiness.
- Full suite + `go vet` + `-race` on `agent`/`server`/root green; `gofmt -l` clean.

## [0.76.88] - 2026-09-21

### Fix — `GET /api/sessions/{id}/info` (and `/{id}`) blocked for the entire duration of an in-flight turn
Reported: both endpoints are documented as fast, read-only snapshots, but built their response from `Session.Meta()` — which takes `s.mu`, the same mutex `promptSync` holds locked from `turn_start` to `turn_end`. A request against a busy session blocked for as long as the turn ran (confirmed live: request time tracked the turn's own duration almost exactly, both in the 300ms range and separately up to several seconds, resolving the instant the turn finished). harness already solved this exact class of problem for three other fields (`CurrentModel()`/`CurrentThinking()`/`CurrentStats()`, all lock-free via `atomic.Value`, built so the `Subagent` tool's executor could read live session state from inside a running turn) — these two endpoints were never updated to use them.

- New `Session.CurrentMaxIterations()` (lock-free, `atomic.Int64`) — `maxIterations` previously had no lock-free counterpart at all, unlike the other three fields, and was read via a PLAIN, unguarded field access (`return s.maxIterations`) despite being written under `s.mu` in `SetMaxIterations` — a genuine data race under Go's memory model that happened not to block only because it skipped the lock rather than contending for it. `MaxIterations()` is now implemented via `CurrentMaxIterations()`.
- New `Session.CurrentMeta()` builds a session-metadata snapshot without ever taking `s.mu`: static fields (ID/CWD/Name/timestamps) come from the store's own short-lived lock (never held for an entire turn), while Model/Thinking/Stats/MaxIterations are overwritten with their existing lock-free accessors so a caller mid-turn sees the CURRENT values. One accepted trade-off: pending delegated cost (`Subagent`/`Fetch` accounting from a background execution) isn't drained here — draining requires `s.mu`, exactly the blocking this exists to avoid; `CurrentStats()` already carries this same staleness window for any concurrent reader.
- `server/server.go`: `handleSessionInfo`, `handleGetSession` (in-memory path), and `handleResumeSession` (idempotent in-memory path) now read through `CurrentMeta()`/`CurrentMaxIterations()` instead of `Meta()`/`MaxIterations()`. Session creation and cold-store resume paths are untouched — a session can never be mid-turn at those points.
- New tests: `TestCurrentMetaDoesNotBlockUnderPromptSyncLock`/`TestCurrentMaxIterationsReflectsSetMaxIterations` (`agent/`, simulate `promptSync` holding `s.mu`, mirroring the existing `TestSessionInfoGettersDoNotDeadlockUnderPromptSyncLock` pattern) and `TestSessionInfoDoesNotBlockDuringInFlightTurn` (`server/`, the suggested acceptance test — starts a real turn, polls until busy, races a real `GET /info` request against it, asserts it returns in the same order of magnitude as any other non-blocking read). Reproduced live: temporarily reverted the fix and confirmed `GET /info`'s response time tracked the turn's own duration almost exactly across repeated runs; restored the fix and confirmed sub-millisecond response regardless of turn length. Full suite + `go vet` + `-race` on `agent`/`server`/root green; `gofmt -l` clean.

## [0.76.87] - 2026-09-21

### DX — `ProviderWithReasoningSplit()` takes no argument; parser-side auto-detection considered and rejected
Explored making `parseOpenAIStream` fully provider-agnostic — relocating `<think>...</think>`-wrapped content into thinking events instead of requiring `ReasoningSplit` opt-in at all. Investigated live against a real gateway proxying to MiniMax with `stream:true`: confirmed the closing tag can arrive concatenated with the start of the real answer in the SAME delta (no clean boundary to key off), which would require a genuine state machine layered onto every OpenAI-dialect provider's shared streaming parser. Rejected: the wire-flag mechanism from v0.76.86 is simpler and already proven correct; not worth the added parsing risk on a code path all providers share. Full rationale: `docs/plans/2026-09-21-custom-providers-design.md`'s Addendum 5.

- `agent.ProviderWithReasoningSplit(v bool)` → `agent.ProviderWithReasoningSplit()` (no argument — calling it always means "on"; omit it to leave the flag off, the default). Same change to the facade.
- **Bug fix caught in the process**: `harness.ProviderWithReasoningSplit` was never aliased in `harness.go` at all when the flag was introduced in v0.76.86 — added it now, and extended `TestNewOpenAIProviderFacadeAliasIsWired` to actually drive a `CompleteStream` call through a facade-registered provider and assert `"reasoning_split":true` reaches the wire (previously untested through the facade).
- Updated call sites/docs: `agent/custom_providers_test.go`, `internal/providers/register_openai_test.go`'s doc comment, `README.md`, `AGENTS.md`.
- Full suite + `go vet` + `-race` on `internal/providers`/`agent`/root green; `gofmt -l` clean.

## [0.76.86] - 2026-09-21

### Fix — custom OpenAI-compatible providers gain opt-in `ReasoningSplit`, extending a known MiniMax fix additively
Continued live-testing a real gateway proxying to MiniMax: turns completed, but thinking never appeared as a separate thinking block — it rendered inline, wrapped in literal `<think>...</think>`, at the start of the answer text. Confirmed live with two direct gateway calls (same prompt, only `reasoning_split` toggled): without the flag, MiniMax puts its full reasoning trace inside `content`; with it, reasoning moves to the dedicated `reasoning_content` field harness already parses into `EventStreamThinkingDelta`. This is the exact same quirk the built-in `minimax` provider already works around (`ReasoningSplit: true`, always on) — `CustomOpenAI` (added later) never had an equivalent field, so no custom provider fronting a MiniMax-flavored backend could opt in.

- New `types.CustomProvider.ReasoningSplit bool` (`omitempty`) — opt-in, `false` by default, so every existing `settings.json` and every custom provider configured before this change is byte-for-byte unaffected.
- `CustomOpenAI.reasoningSplit` threads through `NewCustomOpenAI` (declarative path) and `RegisterOpenAI`'s new `reasoningSplit bool` parameter (SDK path) into `CompleteStream`'s `llm.OpenAIRequest{ReasoningSplit: ...}` — the same field `minimax.go` already sets; this only makes the existing mechanism reachable from the generic custom-provider path, no new wire behavior.
- New `agent.ProviderWithReasoningSplit(bool)` SDK option (`agent/custom_providers.go`), and `--reasoning-split` CLI flag on `harness provider add` (`internal/cli/kong.go` → `settings.go`'s `ProviderAddOpts`/`RunProviderAdd`).
- New tests prove both halves of "additive": `TestCustomOpenAI_ReasoningSplitDefaultsToFalseNotSentOnWire` (key absent when unset), `TestCustomOpenAI_ReasoningSplitSendsWireFlagWhenConfigured` (key present and `true` when set), plus matching coverage through `RegisterOpenAI` and `agent.NewOpenAIProvider`/`ProviderWithReasoningSplit`. Reproduced live: temporarily removed the wiring, confirmed the new tests fail, restored it, confirmed they pass. Full suite + `go vet` + `-race` on `internal/providers`/`agent`/root green; `gofmt -l` clean. Full rationale: `docs/plans/2026-09-21-custom-providers-design.md`'s Addendum 4.

## [0.76.85] - 2026-09-21

### Fix — custom OpenAI-compatible providers accept MiniMax-style clean SSE completion (no `[DONE]` marker)
Found live-testing a real gateway proxying to MiniMax: model discovery worked, but every turn failed in the TUI with `stream ended unexpectedly before completion (no [DONE] marker received) — the connection likely dropped mid-response`. Not a new bug — MiniMax's real API emits a final `finish_reason` chunk then closes its SSE connection without ever sending `[DONE]`, already worked around for the BUILT-IN `minimax` provider via `OpenAIRequest.AllowCleanEOF` (commit `b3f42d7`). `CustomOpenAI.CompleteStream` — added later — never set that flag, so any custom provider fronting a MiniMax-flavored backend (directly, or through a gateway, as here) hit the same false "dropped connection" error on every single turn.
...
- Tests rewritten to prove the new contract (not just updated for the signature change): `TestCustomOpenAI_AlwaysActiveNoCredentials`, `TestCustomOpenAI_ConnectAndDisconnectAreRejected`, `TestFetchCustomOpenAIModels_HeadersCarryCustomAuth` (reproduces a real gateway's header-based auth shape), `TestFetchCustomOpenAIModels_SendsConfiguredHeaders` (now asserts `Authorization` stays empty), `TestRegisterOpenAI_ConnectRejected`, `TestNewOpenAIProvider_AlwaysActiveNoConnectStep`. All green under `go vet`, `go test ./...`, and `-race` on the touched packages; `gofmt -l` clean.

## [0.76.83] - 2026-09-21

### Feature — custom OpenAI-compatible providers (declarative + programmatic)
Two related additions, both landing on the same `internal/providers.CustomOpenAI` (OpenAI's structural twin, but every network-facing detail — name, base URL, models URL, extra headers — comes from configuration instead of being hardcoded). Full design: `docs/plans/2026-09-21-custom-providers-design.md`.

**Declarative (`settings.json` / CLI):**
- New `types.CustomProvider{Type, URL, ModelsURL, Headers, Display, Disabled}` — a new `"provider"` collection in `settings.json` (singular, matching `"mcp"`'s own style), managed via `SettingsManager.CustomProviders()/CustomProvider()/SetCustomProvider()/DeleteCustomProvider()`. Validation (`ErrInvalidCustomProvider`, mapped to 422): `type` must be `"openai"` (only supported dialect for now — `"anthropic"` is out of scope, `Anthropic` has no configurable `baseURL` today), `url` required, and the name must not collide with a reserved built-in provider (`anthropic`, `claude-oauth`, `codex-oauth`, `minimax`, `ollama-cloud`, `ollama`, `openai`, `opencode-go` — see new `config.IsReservedProviderName`).
- New CLI command `harness provider add/list/rm`, distinct from the existing plural `harness providers` (the read-only listing of every registered provider, built-in and custom alike — unchanged, already generic over `providers.All`). `harness connect <name> <key>` already worked generically for any api-key provider, custom included — no changes needed there.
- New HTTP endpoints `GET/PUT/DELETE /api/settings/provider/{name}`, documented in the OpenAPI spec, plus `client.Client.GetCustomProviders/PutCustomProvider/DeleteCustomProvider`.
- `internal/providers/registry.go`'s `initRegistry()` constructs a `CustomOpenAI` for each ENABLED, non-reserved entry (via new `buildCustomProviders`, a pure function taking the map so it's testable without touching the real `~/.harness/settings.json`) — same no-hot-reload contract MCP servers already have (takes effect on the next process start).
- `FetchModels` for a custom provider is a genuinely SEPARATE code path from the built-in `OpenAI` provider's own (`fetchCustomOpenAIModels`, not a parametrized reuse of `fetchOpenAIModels`) — it accepts every model ID an endpoint returns with NO `isOpenAIChatModel` prefix filtering. That filter exists only because the real OpenAI API's `/v1/models` mixes chat models with embeddings/audio/image models and offers no capability field to tell them apart; a custom provider fronting, say, a MiniMax or Llama-model proxy has no such naming convention and would return zero models if that filter ever leaked onto this path. Investigated live whether any standard capability field exists across OpenAI-compatible proxies to filter on instead — it doesn't (LiteLLM, Lemonade, and others each invent their own non-standardized extension) — so unfiltered acceptance is the correct default, not a shortcut.

**Programmatic (SDK):**
- New `agent.NewOpenAIProvider(name, url, apiKey string, opts ...CustomProviderOption) error` (aliased in `harness.go`) registers a custom provider directly in Go code — global to the process (confirmed: `providers.All` has no per-`Agent` isolation, unlike `mcp.Manager` which genuinely is per-`Agent`), with the API key held only in memory (never written to `credentials.json` — same behavior a built-in api-key provider already has when activated via its environment variable instead of `harness connect`, rather than a new mechanism). Call once, early — typically in `main()`, before constructing any `Agent`.
- `agent.ProviderWithDisplay(display string)`, `agent.ProviderWithHeaders(headers map[string]string)`, `agent.ProviderWithFetchModels(fn func(apiKey string) ([]types.ModelMeta, error))` — functional options; the last one fully replaces model discovery (a static list, a differently-shaped endpoint, whatever a deployment needs) instead of the default `<url>/models` HTTP call. No `internal/providers` type appears in any of these signatures — the option accumulator is an unexported struct inside `agent`, only translated to a real `internal/providers.CustomOpenAI` construction inside `NewOpenAIProvider`'s own body.
- New `internal/providers.RegisterOpenAI` is the actual construction/registration entry point `agent.NewOpenAIProvider` wraps — rejects a reserved built-in name or a duplicate registration. `providers.All` gained its first-ever write-protection (`registryMu sync.Mutex`) since `RegisterOpenAI` is a second, arbitrary-timing writer beside `initRegistry`'s original one-time `sync.Once` construction; reads throughout the codebase remain deliberately unprotected under the documented "register everything in `main()` before building any `Agent`" contract.
- `CustomOpenAI` gained an optional `fetchModelsFn` field (checked first in `FetchModels()`, falling back to the default HTTP path when nil) — `types.CustomProvider` itself does NOT gain this field, since it has no JSON representation and only the programmatic path can supply arbitrary Go logic.

Tests (all green under `-race`): `internal/config/settings_test.go` (validation, reserved-name rejection, persistence round-trip, `Disabled` field), `internal/providers/custom_openai_test.go` (DisplayName/ModelsURL fallbacks, headers reaching both chat-completions and models-listing requests, and the critical regression test — accepting `minimax-m3`/`llama-3.1-70b`/`deepseek-chat` unfiltered), `internal/providers/registry_test.go` (`Disabled`/reserved-name/unsupported-type exclusion, deterministic order), `internal/providers/register_openai_test.go` (SDK registration fields, reserved/duplicate-name rejection, the `FetchModels` hook proven via a server that fails the test if actually hit, `Resolve("name/model")` working end-to-end), `agent/custom_providers_test.go` and `harness_test.go`'s `TestNewOpenAIProviderFacadeAliasIsWired`/`TestCustomProviderEndToEnd` (the same coverage through the public `agent`/`harness` packages and real HTTP, isolating `HOME` from the real `~/.harness/settings.json` singleton throughout).
...
- **Field-reported and diagnosed live**: a schedule created in one `harness --scheduler` instance (with its owner session genuinely active there) never fired, while `runs`/`last_run` kept advancing anyway. Root cause: `Engine.evaluate` evaluated and `RecordRun`'d **every** schedule in the shared `~/.harness/schedules.json`, regardless of whether the schedule's owner session was actually active in THAT process. With two `--scheduler` processes running at once (a completely valid, previously-intended setup — e.g. `harness --scheduler` for the TUI and `harness slack --scheduler` for Slack, both alive simultaneously), whichever process's 30s tick happened to land first won the fire+`RecordRun` for every due schedule, even ones it had no session for at all — silently starving the OTHER process's session of a prompt it should have received. Reproduced exactly: a synthetic two-engine test confirmed the "wrong" engine winning both schedules' fires before the fix, and each engine correctly firing only its own after.
- **Fixed** with an inversion of control, as designed: `Engine` (`agent/schedule/engine.go`) now owns a `watched` set of session IDs via new `AddSession`/`DelSession`/`ClearSessions`/`Watching` methods (own mutex, independent of the store's). `evaluate` skips any schedule whose owner isn't in the watched set — before parsing its cron, before firing, before `RecordRun` — leaving it entirely untouched for whichever OTHER engine (if any) does watch that owner. A freshly constructed engine watches nothing and does nothing until told otherwise.
- `Agent.registerSession`/`unregisterSession` (`agent/agent.go`) — already called by `NewSession`/`ResumeSession`/`ForkSession`/`Session.Close` — now also call `schedEngine.AddSession`/`DelSession` (no-op when `schedEngine` is nil, i.e. `EnableScheduler` off). No new call sites needed: every session an instance creates/resumes/forks/closes automatically keeps the engine's watched set in sync with reality.
- **Net effect**: several `harness` processes can now run `--scheduler` at once, each safely firing only into the sessions it itself has live — the deployment shape the code already claimed to support but didn't actually enforce. `fireScheduledPrompt`'s existing drop-if-inactive behavior (v0.76.79) is now backed by a filter that prevents the race from reaching it in the first place, rather than only catching it after another process's engine already won.
- Tests: `agent/schedule/engine_watch_test.go` (new) — unwatched schedules never fire or `RecordRun`, multi-schedule filtering, `DelSession`/`ClearSessions` take effect immediately, `AddSession("")` is a no-op (never wildcards the stale-owner sentinel), a fresh engine starts silent, the two-engine race reproduction, and a `-race`-covered concurrent `AddSession`/`DelSession`-during-`evaluate` check. `agent/schedule/store_test.go`'s 4 existing engine tests updated to call `AddSession` explicitly (confirms the filter is real — they failed without it). `agent/schedule_gating_test.go` gained 3 wiring tests confirming `NewSession`/`Close` actually reach the live engine's watched set, not just that `Engine`'s own API works in isolation.
- `harness.go`'s `AgentWithScheduler` doc comment and `agent/agent.go`'s `fireScheduledPrompt`/`Engine` comments updated — the old "only one instance should run --scheduler" language is gone; multiple instances are now an explicitly safe, intended shape.
...
- `SearchMessages(sessionID, query, limit) ([]SearchResult, error)` is now a first-class method on the `SessionStore` interface (`agent/store`), alongside `AppendMessage`/`LoadMessages`/`TruncateMessages` — full-text search over a session's message log is persistence logic, not tool logic, and any future custom `SessionStore` (Postgres, S3, a hosted backend) can now implement its own search strategy directly instead of having no hook at all.
- `FileStore.SearchMessages` (`agent/store/file.go`) now owns everything that used to live in `agent/tools/session.go`: the per-session `<session-id>.search.db` FTS5 index (same on-disk location as before), schema migration, the `filter_version`-guarded rebuild for the compaction/system-generated exclusion filter (v0.76.74), and the `busy_timeout(5000)`+`journal_mode(WAL)` DSN pragmas (v0.76.73). `InMemoryStore.SearchMessages` returns the new `store.ErrSearchNotSupported` sentinel — no in-memory FTS reimplementation, by design.
- **Reverted to SQLite's native `snippet()`** (`snippet(messages_fts, 1, '[', ']', '...', 64)`, 64 tokens — the hard ceiling SQLite itself enforces) instead of v0.76.73's `highlight()` + hand-rolled `windowSnippet()` character-window approach. Deliberate simplicity trade-off: less roomy than the ~500+ char window, but zero custom trimming code to maintain, and SQLite's own match-centering heuristic instead of a hand-rolled one. `windowSnippet()` is deleted, not carried over.
- **Concurrency, done right this time**: a dedicated `map[sessionID]*sync.Mutex` on `FileStore` guards only `SearchMessages`, completely independent of the store's existing `mu` (which guards meta/log I/O for ALL sessions) — so a slow search on one session never blocks any other session's reads/writes. Confirmed live in the design phase that `busy_timeout+WAL` alone is NOT sufficient even for this narrower per-session scope: re-running the v0.76.73 concurrency probe (N goroutines racing to bootstrap a brand-new SQLite file) still hit `SQLITE_BUSY` 3–8 times per 50 rounds at 2–4-way concurrency with the pragmas already set — the race is specifically in the very first `CREATE TABLE`/`CREATE VIRTUAL TABLE` migration, which `busy_timeout` doesn't reliably protect.
- **New**: `TruncateMessages` now also deletes the session's `.search.db` (and `-wal`/`-shm` sidecars) when present — after a `/reset`, the index's message-count offset would otherwise point at content that no longer exists; deleting and letting the next `SearchMessages` call rebuild from scratch is simpler and more obviously correct than reconciling a stale offset.
- `agent/tools/session.go`'s `SessionSearch` collapses to a thin adapter: parse/validate input, clamp `limit`, delegate to an injected `SessionSearchFunc`, translate `store.ErrSearchNotSupported` into a plain user-facing error. No more SQL, schema, or locks in `agent/tools` at all. `agent/agent.go`'s wiring shrinks from two injected closures (a message provider + an index-path resolver) to one (`Session.SearchMessages`); the now-unnecessary `sessionSearchIndexPath` helper is deleted.
- Tests: the full FTS5 test suite (snippets, concurrency, compaction/system-generated exclusion, filter-version rebuild) moves to `agent/store/search_test.go`, now exercising `FileStore.SearchMessages` directly; `agent/tools/session_test.go` shrinks to the tool's own responsibilities (parsing, clamping, delegation, error translation) against a fake `SessionSearchFunc`; new tests cover `InMemoryStore`'s `ErrSearchNotSupported`, the new `TruncateMessages` index-deletion behavior, that different sessions' searches never block each other, and a dedicated deadlock regression (`TestSessionSearchMessagesDoesNotDeadlockUnderPromptSyncLock`) mirroring `SessionInfo`'s own.
- Design doc: `docs/plans/2026-09-16-sessionsearch-into-store-design.md`.
...
- `harness` failed to create a session on Windows with `mkdir C:\Users\alice\.harness\agent\sessions\C:-Users-alice-Downloads: The directory name is invalid.`
- **Root cause** (`agent/store/file.go`): `cwdSlug` replaced the OS path separator with `-` but left the Windows drive-letter colon (`C:`) untouched, folding it directly into the directory NAME instead of its normal drive-separator position — `C:\Users\alice\Downloads` slugged to `C:-Users-alice-Downloads`, a component NTFS rejects outright.
- **Fix**: `cwdSlug` now strips every NTFS-illegal path character (`:<>"|?*`), unconditionally on any OS — not just when `GOOS=="windows"`, since a slug should be safe to recreate on any platform a session store might later be read from. `C:\Users\alice\Downloads` now slugs to `C-Users-alice-Downloads`.
...
- `harness connect claude-oauth` failed at the token-exchange step with `400 invalid_request_error: "Invalid request format"` after the OAuth-behind-the-API migration (v0.76.69).
...
- Disconnecting both `minimax` and `ollama-cloud` mid-session did not stop `WebSearch` from working — it kept using credentials resolved when the session was first built.
...
- `ANTHROPIC_CLI_VERSION` names the wrong thing — the value it overrides is the CLAUDE CODE client version harness impersonates in its `user-agent` header (see 0.76.63's fix), not an Anthropic API version. `CLAUDE_CLI_VERSION` names what it actually controls.
...
- For research/investigation/code-review sub-agent tasks that should never modify anything, an explicit, structural way to prevent it — not just relying on the caller's own prompt telling the sub-agent "don't write anything".
...
- Follow-up to 0.76.60/0.76.61's `CGO_ENABLED=0` fix: that fix was described as "graceful degradation", but auditing the actual code proved that wrong — `golang.design/x/clipboard`'s non-Windows, cgo-less backend (`clipboard_nocgo.go`) doesn't return an error on failure, it `panic()`s outright on every call (`Init`/`Read`/`Write`/`Watch`). Since `pasteClipboardImage` (`internal/tui/layout.go`) invokes the clipboard from inside its own goroutine, an unrecovered panic there would crash the ENTIRE harness process — Go does not let one goroutine's panic be caught from another. Verified experimentally: a small standalone program confirmed `clipboard.Init()` panics (not errors) under `CGO_ENABLED=0`, and a new regression test reproduced the exact crash against harness's own code before this fix (and passed cleanly after).
- **Fix — the crash itself** (`internal/tui/clipboard.go`): both `initClipboard()` and `PasteImageFromClipboard()`'s `goclip.Read()` call are now wrapped with `recover()`. A panic (which is exactly what any `CGO_ENABLED=0` build produces the instant clipboard access is attempted) is now treated identically to any other init failure — `clipReady` stays false, the caller gets the same "clipboard not available on this system" error, and the process keeps running.
- **Fix — the underlying trade-off, reconsidered per-platform, not blanket** (`.github/workflows/release.yml`): `linux/amd64` — the realistic desktop-clipboard-usage case among the cross-compiled release binaries — now builds with **real cgo enabled**, via one `apt-get install libx11-dev` step. This works with no cross-toolchain complexity because `clipboard_linux.c` `dlopen()`s `libX11.so` at RUNTIME rather than linking against it at build time — only the headers are needed to compile. The other 4 targets stay `CGO_ENABLED=0`, each for a distinct, now-documented reason: `linux/arm64` would need a full multiarch cross-toolchain (`dpkg --add-architecture arm64` + `ports.ubuntu.com` sources + `gcc-aarch64-linux-gnu`) for a target that's realistically Raspberry Pi/ARM **servers** run headless over SSH, where an X11 clipboard wouldn't function anyway even with cgo; `darwin/{amd64,arm64}` need the Cocoa/Foundation frameworks (`clipboard_darwin.m`), which aren't cross-compilable from a Linux runner regardless of cgo settings; `windows/amd64` never used cgo for this in the first place (`clipboard_windows.go` is pure `syscall`/DLL calls).
- Tests: `internal/tui/clipboard_test.go` (new) — `TestPasteImageFromClipboardNeverPanics` exercises the REAL code path (not a mock) and is explicitly designed to be run under `CGO_ENABLED=0` to reproduce the exact crash condition; confirmed to fail against the pre-fix code (via `git stash`) and pass against the fix, in both the default (cgo-enabled) and `CGO_ENABLED=0` local test runs. Full suite + `-race` + `go vet ./...` green in both modes.
...
- Publish real GitHub Releases (binaries attached, not just a bare git tag) automatically whenever `make release-push` pushes a version tag, plus a manual way to (re)build one from the Actions tab.
- **New** (`.github/workflows/release.yml`): a single workflow with two triggers — `push: tags: 'v*'` (automatic, fires the instant `make release-push` pushes a tag) and `workflow_dispatch` with an optional `tag` input (manual; empty defaults to the latest existing tag, useful for a re-run after a transient CI failure or to backfill assets for an already-tagged release). Both paths converge on the same job: checkout the resolved tag, build 5 platform/arch combinations (`darwin/amd64`, `darwin/arm64`, `linux/amd64`, `linux/arm64` — new, for Raspberry Pi/ARM servers — and `windows/amd64`), package each as `.tar.gz` (Unix, preserves the executable bit) or `.zip` (Windows), generate a `SHA256SUMS.txt`, and publish a GitHub Release with the exact `## [X.Y.Z]` section from `CHANGELOG.md` as its notes (same extraction `make release` already uses for the git tag message — one source of truth for release notes either way). `--clobber` lets a manual re-run replace an existing release's assets instead of failing. All dynamic/untrusted values (the manual dispatch's free-text `tag` input, resolved tag names) are passed through `env:` rather than interpolated directly into `run:` scripts — hardening against the documented GitHub Actions script-injection risk of `${{ }}` expressions landing in shell code.
- **`Makefile`**: `build-all` gained the 5th platform (`linux/arm64`), kept in sync with the workflow's own build matrix so the same 5 targets are produced whether run in CI or locally.
- **Real bug found and fixed while validating this**: `make build-all` had never actually succeeded for Windows — `internal/tui/term/terminal.go` called `syscall.SIGWINCH` with no build tag, and that signal doesn't exist in Windows's `syscall` package, so `GOOS=windows go build ./...` failed outright. Fixed by extracting the resize-signal wiring behind the same `//go:build unix` / `//go:build windows` / `//go:build !unix && !windows` three-way split the project already uses for `agent/tools/bash_*.go`: new `internal/tui/term/resize_{unix,windows,other}.go`, each providing `notifyResize() chan os.Signal` / `stopResize(chan os.Signal)`. Windows's `notifyResize` returns `nil` — a nil channel blocks forever in `resizeLoop`'s `select` (documented Go behavior), which is exactly the right no-op: the TUI works fine on Windows, a manual terminal resize just doesn't trigger a live re-render until the next natural redraw. Verified via `GOOS=<os> GOARCH=<arch> go build ./...` for all 5 targets, plus running the packaged `darwin/arm64` binary extracted from its own `.tar.gz` end-to-end.
- Full suite + `-race` + `go vet ./...` green across the native platform; cross-compilation verified for all 5 release targets.

## [0.76.59] - 2026-09-04

### Change — `skill:<name>` echo format tightened to `Skill:<name>`, no space
- The previous fix's echo format was `"Skill: goal <text>"` (space after the colon) — changed to `"Skill:goal <text>"` (no space), mirroring the literal `skill:<name>` command syntax instead of reading as a separate word.
...
- After using `/goal` in practice: the dedicated `Goal` tool's fixed role prompts (`builderRolePrompt`/`testerRolePrompt`) implicitly treated the Builder/Tester sub-agents as near-blank slates, pushing the parent to over-explain project context those sub-agents already have full access to (same cwd, same tools, same skills, same read-only memory — see the "sub-agents share parent's project access" fix later in the v0.76.56 changelog entry, which was already pointing at this same tension). Once the protocol was refined externally into something far more rigorous (a frozen Acceptance Contract, mandatory goal-hacking detection in the Tester, a round ledger, concrete stall criteria), it became clear the protocol had grown too large and too much a "living procedure that should be editable without a recompile" to keep justifying its own purpose-built tool, hardcoded Go role prompts, and a dedicated `/goal` server command.
- **Change**: the entire protocol now lives as a **Skill** — `~/.harness/agent/skills/goal/SKILL.md` — orchestrated by the model itself making two plain, sequential, foreground `Subagent` tool calls per round (Builder, then a FRESH Tester every round) instead of a purpose-built `Goal` tool. Invoked via the already-existing generic `skill:<name>` command path (`server.go`'s `handleExecCommand` default case) — no new server-side command, no new tool, no new prompt-injection mechanism. The skill itself carries everything the old fixed role prompts + `GoalCommandPrompt` used to hardcode in Go: the Orchestrator framing, the 5-phase protocol (frame the run → Builder → Tester → adjudicate → escalate-or-repeat), required report formats, and — new, not present before — a frozen Acceptance Contract the model may never weaken to force a PASS, a mandatory goal-hacking check (inspecting the diff for weakened tests/disabled checks/hardcoded values to fake a pass), concrete stall-detection criteria (no criterion flipped FAIL→PASS in 2 rounds, or the same criterion FAILs twice with no new information), and a round ledger passed to each Builder from round 2 on.
- **Known trade-off, accepted deliberately**: the old `Goal` tool technically enforced Tester/Builder separation of duties via `DisallowedTools: [Write, Edit]` on the Tester's ephemeral sub-agent — a real code-level guarantee. Plain `Subagent` calls have no such mechanism; "the Tester never edits, only reports" is now a prompt instruction inside the skill, not a technical barrier. Accepted as the right trade-off given the skill's own new goal-hacking check specifically targets exactly this failure mode from the other direction (catching a Tester or Builder that tampered with verification rather than preventing the tampering outright).
- **Removed entirely**: `agent/tools/goal.go` (the tool), its executor/wiring in `agent.go` (including the `runRole` helper and all `ToolGoal` references in `DisallowedTools` lists), `ToolGoal` (`agent/tools/names.go`), `GoalCommandPrompt` (`agent/prompts.go`), the `"goal"` command (`server.go`'s `handleExecCommand` case + its `var commands` entry), the `Goal`-specific spinner handling and `✪` icon (`internal/tui/commands.go`, `internal/tui/output.go`), and `PromptWithDisplayText`/`Session.followUp.displayText` (`agent/session.go`) — a mechanism added specifically to keep `Goal`'s large fixed protocol prompt from leaking into the TUI's prompt echo, now unnecessary since `skill:goal`'s injected prompt is short (skill location + skill content + the user's own `/goal <objective>` text) and echoing it plainly is fine. All corresponding tests removed (`agent/tools/goal_test.go`, `agent/goal_test.go`, the `Goal`/`PromptWithDisplayText` entries in `schema_audit_test.go`/`validate_migration_test.go`/`prompt_option_test.go`).
- The original design doc (`docs/plans/2026-08-31-goal-tool-adversarial-loop-design.md`) is marked SUPERSEDED with a pointer to this entry, kept for historical context rather than deleted.
- **Critical follow-up on the skill's own `description`**: a skill's frontmatter `description` is the ONLY thing the model evaluates before deciding to auto-load it (the body is only read after that decision) — and mentioning words like "adversarial", "build", "test", "verify" risked the model self-triggering this expensive, multi-round workflow on an ordinary task that merely mentions building or testing something. Refined to be short and explicit: what it does in one sentence, then a standalone "Heavyweight (multiple sub-agent rounds) — NEVER load this on your own initiative" — with no elaboration on exactly when TO use it (redundant: `skill:goal` is itself a user-initiated command, so the auto-trigger risk only exists in the OTHER direction). An earlier, longer draft (dropped) also referenced the now-removed `/goal` command by name — cut as dead text once the dedicated command was removed.
- Verified live: `GET /api/sessions/{id}/commands` correctly discovers `skill:goal` with the skill's frontmatter `description`, sourced entirely from the generic skill-loading machinery with zero `Goal`-specific server code remaining. Full suite + `-race` + `go vet ./...` green (aside from the same pre-existing, unrelated `TestSubagentMaxIterationsOverrideReachesEphemeralSubAgent` model-compliance flake noted in earlier entries).

## [0.76.56] - 2026-08-31

### Feature — `Goal` tool + `/goal` command: an adversarial Builder/Tester loop for larger, self-verified tasks
- A Claude-Code-style `/goal <prompt>` command driving an adversarial build/test loop — a Builder sub-agent implements, an independent Tester sub-agent verifies against acceptance criteria, and failures feed back into the next Builder round until everything passes. Researched the agentic literature before designing (Generator-Critic / Evaluator-Optimizer, production Builder/Adversary loop pairs) — see `docs/plans/2026-08-31-goal-tool-adversarial-loop-design.md` for the full design, including two rejected alternatives (a fully Go-driven loop, and pure prompt-only orchestration) and why the load-bearing rules from the research (context isolation between roles, per-criterion structured verdicts, "no changes needed" as a first-class outcome, no critic edit rights, an explicit round/budget cap) shaped the final design.
- **Core design decision**: `Goal` (`agent/tools/goal.go`) runs exactly **ONE round** of Builder→Tester, blocking (foreground only, no `background`) — the round-trip LOOP lives in the parent's own ReAct loop, which reads the Tester's full report each round and decides (with its own live session context — prior decisions, project conventions, memory) how to refine the next round's `builder_prompt`, or whether the goal is met. This avoids both rejected alternatives: a Go driver loop has no way to originate/refine a good brief without an LLM in the loop anyway, and prompt-only polling burns real tokens/turns just checking "is it done yet?".
- **Input**: `builder_prompt` (operational instructions for the Builder), `tester_prompt` (a list of specific, checkable acceptance criteria — not a vague instruction), `timeout` (default 600s — much larger than `Subagent`'s 120s, covering the round's FULL wall-clock time across BOTH sub-agent calls on a single shared deadline, never split or reset between them), `max_iterations` (shared ReAct budget for both sub-agents this round, same `[1,200]` validation contract as `Subagent`'s own override).
- **Fixed role prompts** (`builderRolePrompt`/`testerRolePrompt`, never touched by the caller) define HOW each role behaves and the handoff format: the Builder ends with a fixed `## Builder Report` (changes made, files touched, how to verify); the Tester receives that report verbatim plus the acceptance criteria and must respond with a `## Test Report` evaluating EACH criterion separately (never collapsed into one score) plus a final `Overall: PASS`/`Overall: FAIL` line.
- **Separation of duties**: the Tester sub-agent is built with `Write`/`Edit` disallowed on top of the same recursion/memory/schedule restrictions every `Goal`/`Subagent` ephemeral sub-agent gets — a Tester that can edit stops being an independent judge and starts silently "fixing" instead of reporting.
- **The corrected error contract** (caught during design review): a `FAIL` verdict is normal, successful tool output — valid business information, not a system failure — so it returns `(testerReport, nil)`, never a tool error. `Goal` never parses or acts on the verdict itself; only a genuine system failure (a sub-agent erroring, a timeout via the same `isTimeout` used by `Subagent`/`ColleagueAsk`/`Fetch`, or `ctx` cancellation) returns a real error, discarding any partial output exactly like those tools already do.
- **`agent.go` wiring**: the executor runs the Builder first (full tool access, same as a normal `Subagent`), then the Tester (Builder's restrictions + no Write/Edit) sequentially on the SAME `ctx` — never parallel, since the Tester needs the Builder's finished report as input. Both ephemeral sub-agents' `Stats()` fold into the parent session's totals via the existing `addDelegatedCost` mechanism (same as `Subagent`/`Fetch`).
- **`/goal <prompt>` command** (`server/server.go`'s `handleExecCommand`, registered in the dynamic session-command list): doesn't run a new server-side state machine — like `skill:<name>`, it composes one fixed prompt (`agent.GoalCommandPrompt`, `agent/prompts.go`) explaining the protocol at a high level (compose this round's prompts, read the Tester's verdict, refine and retry on FAIL, stop and ask the user if progress stalls or the objective turns out infeasible) and injects it via the session's normal `Prompt()` — the model's own ReAct loop, driven by repeated `Goal` tool calls, IS the round-trip loop from there. `Goal`'s own tool `Description` explicitly forbids self-initiated use ("only when the user has explicitly requested this... via the '/goal' command").
- **TUI**: `/goal` gets the same spinner-instead-of-static-confirmation treatment as `/compact`/`skill:` (`internal/tui/commands.go`) — but unlike `compact` (which has its own dedicated `compact_end` event), `/goal`'s spinner has no dedicated end event and turns off via the same generic `turn_end` path any normal multi-tool-call prompt already uses, since a `/goal` invocation is exactly that: one parent turn that may contain several `Goal` round-trips before finishing. New `✪` (star-in-circle) icon in the tool header (`internal/tui/output.go`) — deliberately NOT crossed swords: the icon should read as "goal/objective", not the internal adversarial mechanism used to get there.
- Tests: `agent/tools/goal_test.go` (17 unit tests against a mock executor — validation, timeouts, the shared-deadline contract, `max_iterations` range, role-prompt assembly, and the critical `TestGoalFailVerdictIsNotAToolError`), `agent/goal_test.go` (live end-to-end integration against a real connected provider — an achievable criterion and a deliberately impossible one, both confirming `IsError=false` regardless of verdict; verified stable across repeated fresh runs), plus `Goal` added to the existing `schema_audit_test.go`/`validate_migration_test.go` cross-tool audits. Full suite + `-race` + `go vet ./...` green.

### Fix — `/goal`'s injected protocol prompt was leaking verbatim into the TUI, and its tool render borrowed `ColleagueAsk`'s bold/dim treatment incorrectly
- The TUI's prompt echo showed the ENTIRE fixed protocol text `agent.GoalCommandPrompt` composes (multi-paragraph instructions the model needs, never meant for a human to read) instead of just the user's own goal — because the TUI's echo path is a strict "backend is the single source of truth" mirror of whatever text the session actually sends to the model (`EventReceivedPrompt`/`EventFollowUpStart` carry `fu.text` verbatim), and `/goal`'s handler had no way to separate "what the model needs" from "what a human should see" before this fix.
- **Fix — `PromptWithDisplayText` (`agent/session.go`)**: a new `PromptOption` that overrides ONLY what `EventReceivedPrompt`/`EventFollowUpStart` echo to transports, without touching what the model receives — `followUp`/`promptConfig` gained a `displayText` field; the echo site now emits `fu.displayText` when set, `fu.text` otherwise (today's behavior for every other caller, unchanged). `server.go`'s `"goal"` command handler now calls `proxy.session.Prompt(ctx, agent.GoalCommandPrompt(goalText), agent.PromptWithDisplayText("Goal: "+goalText))` — the TUI now echoes `❯ Goal: <the user's own text>`, while the model still receives the full injected protocol prompt unchanged. Deliberately implemented at the `agent.Session` level (not a TUI-only patch): it's general SDK behavior any future caller composing a large "protocol" prompt around a human-supplied fragment can reuse, and it preserves the existing "backend is the single source of truth for echoing" invariant instead of working around it with a client-side suppression flag (which would have raced against the async SSE echo, especially with queued prompts).
- **Fix — `Goal`'s tool-call render (`internal/tui/toolfmt.go`)**: removed `Goal` from the `primaryParam`/`secondaryPrimaryParam` maps — it had been given the same "bare primary (bold) + bare secondary (dim)" treatment as `ColleagueAsk` (designed for a short "who" + a long "what", e.g. colleague name + prompt), which read wrong for `Goal`'s two comparably-sized prompt blocks (`builder_prompt`/`tester_prompt`). With no entry in either map, `Goal`'s args now render through the fully generic `key=value` path every MCP tool already uses — no bold/dim asymmetry between the two prompts.
- Tests: `agent/prompt_option_test.go` gained `TestBuildPromptConfigDefaultsToEmptyDisplayText` and `TestPromptWithDisplayTextSetsOverride`. Full suite + `-race` + `go vet ./...` green (aside from the pre-existing, unrelated `TestSubagentMaxIterationsOverrideReachesEphemeralSubAgent` model-compliance flake, confirmed reproducible independently of this change across repeated fresh runs).

### Fix — `Goal`'s icon read as "internal combat", not "objective"; `GoalCommandPrompt` didn't say WHEN to gather context
- The `⚔` (crossed swords) icon communicated the adversarial MECHANISM (Builder vs. Tester), not the user-facing concept of a goal/objective being pursued. Changed to `✪` (star-in-circle) in `internal/tui/output.go` — reads as "objective", doesn't collide with any existing tool icon.
- **Design gap found via review**: `GoalCommandPrompt` never told the parent WHEN to gather context relative to calling `Goal` — its wording ("compose builder_prompt from... everything you **already** know") implicitly nudged the model to skip straight to round 1 from assumptions, observed in practice on a real `/goal` run (a 30-city news-gathering task) where the parent went directly to `Goal` with no research step of its own. Since the Builder and Tester are both EPHEMERAL sub-agents with zero memory of the conversation, skipping this means any research they end up needing gets redone from scratch inside their own (separately budgeted) iterations — and can diverge between the two, since nothing guarantees the Builder and Tester arrive at the same facts independently.
- **Fix** (`agent/prompts.go`): `GoalCommandPrompt` gained an explicit but **conditional** Step 0 — check first whether the objective is already fully answerable from what the parent knows (a simple task, or something already covered earlier in the conversation); if so, skip straight to step 1 with zero overhead. Only if something's genuinely missing (an unfamiliar codebase area, a file not yet looked at, an unverified fact) does it gather that using the parent's OWN tools (Bash/Read/Fetch/MemoSearch/etc.) BEFORE composing round 1's `builder_prompt`/`tester_prompt` — re-affirmed at the refine-and-retry step (5) too, so a `FAIL` round that reveals a new gap triggers the same conditional gather-first check before the next `Goal` call. The parent doing this legwork once, itself, ONLY when needed, and handing both sub-agents a complete, concrete, already-scoped brief is cheaper and more consistent than either sub-agent investigating on its own — while never forcing busywork on objectives that don't need it.
- No test changes needed — `GoalCommandPrompt`'s only test-surface user (`server.go`'s `"goal"` command handler) has no test asserting its literal wording. Full suite + `-race` + `go vet ./...` green.

### Fix — `Goal` discarded the Builder's actual deliverable, risking hallucinated content on content-producing tasks
- Caught in a real transcript: after a `/goal` run to gather news for several cities, the parent's own thinking showed it realizing it had the Tester's verdict ("Overall: PASS") but NOT the actual news content — the Builder's response, containing the real deliverable, was never returned by `Goal`. The parent was about to reconstruct/guess the content instead of having the genuine text — a real hallucination risk directly caused by the tool's own contract.
- **Root cause**: `Goal` was designed to return only the Tester's report (the verdict decides whether to iterate — see the FAIL-is-not-an-error fix earlier in this same release). That's correct for code changes, where the deliverable lives in files the caller can `Read` back anytime independent of what `Goal` returns. It breaks for content-producing tasks (a written report, an analysis, anything not persisted to disk): the Builder's response is the ONLY place that content ever exists, and discarding it after the Tester consumes it as evidence loses it permanently.
- **Fix** (`agent/tools/goal.go`, `agent/agent.go`): new `GoalRoundResult(builderReport, testerReport)` assembles the tool's actual return value as BOTH reports — the Builder's full raw response, a blank line, then the Tester's report untouched. No extra wrapping header is added around the Builder's response: `builderRolePrompt` already makes it end with its own `## Builder Report` section, which is already a clear enough delimiter on its own. Costs nothing new: the Builder's response was already paid for in tokens once (it's handed to the Tester as evidence) — surfacing it to the caller too is free reuse. `Goal`'s tool `Description` updated to tell the model to use the Builder's response as the real content when reporting back to the user, never reconstruct it from the Tester's summary.
- Tests: `agent/tools/goal_test.go` gained `TestGoalRoundResultIncludesBothReports` and `TestGoalRoundResultDoesNotAddAnExtraHeader`; `agent/goal_test.go`'s live integration test now also asserts the Builder's own `## Builder Report` section is present in a real round's output. Full suite + `-race` + `go vet ./...` green.

### Fix — the parent kept redundantly restating each sub-agent's role inside `builder_prompt`/`tester_prompt`
- Observed tool calls like `builder_prompt="You are the Builder in a build/test loop.\n..."` / `tester_prompt="You are the Tester in an adversarial build/test loop.\n..."` — the parent duplicating role-setting text the tool ALREADY establishes internally via the fixed `builderRolePrompt`/`testerRolePrompt` (see the earlier `Goal` feature entry in this release). Neither the tool's `Description`/schema nor `GoalCommandPrompt` had ever told the model those roles were already covered, so it filled that gap with its own assumption — reasonable in isolation, but wasteful and liable to drift from the tool's own fixed wording.
- **Fix** (`agent/tools/goal.go`): `Goal`'s `Description` now states explicitly that the tool already establishes each sub-agent's role/behavior internally, and that `builder_prompt`/`tester_prompt` must contain ONLY operational content — never role-setting text like "You are the Builder/Tester...". Both params' schema `description` fields carry the same explicit prohibition — this is the single source of truth for the rule.
- **Follow-up** (`agent/prompts.go`): `GoalCommandPrompt`'s steps 1/2 originally repeated the same "don't include role-setting text" explanation inline, near-verbatim between the two steps — redundant with the tool's own `Description`/schema (already the authoritative source) and with each other. Trimmed both steps back down to their essential instruction (compose operational content / a checkable criteria list), relying on the tool's own schema to carry the "no role-setting text" rule instead of restating it a third time.
- **Second follow-up** (`agent/tools/goal.go`): the same duplication had crept into the tool's OWN schema too — the "no role-setting text" rule was spelled out three times with three different wordings (once in `Description`, once in `builder_prompt`'s own `description`, once in `tester_prompt`'s own `description`). Consolidated to a single authoritative statement in `Description` (covering both params, with both concrete examples — "You are the Builder..."/"You are the Tester..."); each param's own `description` now just points back to it ("Operational content only — see the tool description") instead of re-deriving the rule. General principle applied: a constraint shared by multiple params belongs in the tool-level `Description` exactly once, not repeated per param.
- No test changes — this is prompt/schema wording only, verified via a live end-to-end run confirming the model still avoids role-setting text with the trimmed schema. Full suite + `-race` + `go vet ./...` green.

### Change — the Builder/Tester and `GoalCommandPrompt` treated ephemeral sub-agents as blanker slates than they actually are
- The parent was implicitly expected to gather and hand over near-complete project context before ever calling `Goal` — redundant, since the Builder/Tester sub-agents run in the SAME project as the parent (same cwd, same tools — Bash/Read/Fetch/etc. — same skills, same read-only memory search) via `runRole`'s `subAgent.NewSession(cwd, ...)` in `agent.go`. They're only missing two things: this specific conversation's history, and the ability to ask questions. Everything about the PROJECT itself, they can discover on their own exactly as the parent could.
- **Fix** (`agent/prompts.go`): `GoalCommandPrompt`'s step 0 narrowed from "gather whatever context is missing" (implicitly inviting the parent to re-derive project knowledge a sub-agent could find itself) to specifically "what only exists in THIS CONVERSATION's history — a decision already made, a constraint the user stated, something already tried and ruled out." Explicitly tells the parent not to re-discover or restate project context the sub-agents can explore themselves.
- **Fix** (`agent/tools/goal.go`): `builderRolePrompt`/`testerRolePrompt` now explicitly tell each sub-agent it has the same project access as anyone else here and should use it — read files, search the codebase, run commands, check conventions — rather than assuming or working only from what the prompt handed it. `builderRolePrompt` frames anything conversation-specific the parent includes as the one thing it genuinely has no other way to know; everything else, it's told to investigate.
- **Also fixed in passing**: `testerRolePrompt`'s required response format had drifted to `## Test Veredict` (a typo, discovered because it broke `TestGoalTesterPromptIncludesCriteriaAndBuilderReportVerbatim`) — corrected back to `## Test Report`, matching every other reference to it (`GoalRoundResult`, the tool's `Description`, all tests).
- Full suite + `-race` + `go vet ./...` green (aside from the same pre-existing, unrelated `TestSubagentMaxIterationsOverrideReachesEphemeralSubAgent` model-compliance flake noted earlier in this release — reconfirmed independent of this change across 3 fresh runs).

## [0.76.55] - 2026-08-27

### Change — `Subagent` gains an optional `max_iterations` override for large delegated tasks
- The default sub-agent budget (`subagentMaxIterations`, 50) is tuned for a typical focused delegated task, but a genuinely large/multi-step one (extensive codebase exploration, many files/URLs, a long multi-phase investigation) could get cut short with no way for the calling model to ask for more room.
- **Change** (`agent/tools/subagent.go`): `subagentInput` gains `max_iterations` (int, omitempty). `0` (the JSON zero-value, indistinguishable from "omitted") means "use the default"; any other value is validated to fall inside `[subagentMinIterations=1, subagentMaxIterationsCeiling=200]` and **rejected outright** (not silently clamped) if it doesn't, with an actionable error message — this applies identically to both the blocking and `background: true` paths. `SubagentExecutor`'s signature grew a `maxIterations int` parameter accordingly; `runSubagentBackground` now accepts and forwards it into its goroutine's executor call. The tool's `Description`/schema documents the new param and explicitly nudges against overusing it ("leave unset for a typical, focused delegated task").
- **Change** (`agent/agent.go`): the `Subagent` executor closure (`buildSessionTools`) now computes `maxIter := subagentMaxIterations; if requestedMaxIterations > 0 { maxIter = requestedMaxIterations }`. The override is deliberately **NOT** capped by the parent session's own `maxIterations` — the earlier `min(parentA.maxIterations, subagentMaxIterations)` is gone. A sub-agent is an independent, separately-budgeted execution (especially relevant for `background: true`, which exists precisely to let a large task run detached from the parent's own turn); the only ceiling that still applies is the tool's own absolute safety limit, already enforced before this closure ever runs.
- Tests (`agent/tools/subagent_test.go`): omitted `max_iterations` passes `0` through unchanged; a valid value reaches the executor; both range boundaries (1 and 200) are accepted; out-of-range values are rejected in both foreground and `background: true` modes without ever reaching the executor. Integration test (`agent/delegated_cost_test.go`, `TestSubagentMaxIterationsOverrideReachesEphemeralSubAgent`) verifies the override actually reaches the ephemeral sub-agent `buildSessionTools` constructs (not just that the tool validates/forwards it) — since that sub-agent discards its state on `Close()` and never propagates its own events to the parent, it forces an observable behavior difference instead: requesting `max_iterations=1` (zero real ReAct passes) for a task requiring a tool call first, and checking the final answer's SHAPE (a short genuine completion vs. a structured multi-paragraph "hit my limit" report) rather than a fragile substring match, since a legitimately-pending report can itself quote the target completion phrase. Verified stable across 6 fresh (uncached) live runs against a real connected provider. Full suite + `-race` + `go vet ./...` green.

## [0.76.54] - 2026-08-27

### Change — `Subagent` and `Fetch`'s prompt-condensing sub-agent now roll their token/cost spend into the parent session's totals
- Delegated execution (a `Subagent` call, or `Fetch`'s `prompt`-driven summarizer sub-agent — see v0.76.51) runs in a separate, ephemeral `*Session` with its own isolated context window, discarded on `Close()` — so its real token/dollar spend used to simply vanish, invisible in the launching session's own `Stats()`/`Meta()`/billing totals, even though the money was genuinely spent on that session's behalf.
- **Change**: a new, deliberately **private** `Session.addDelegatedCost(delta types.SessionStats)` (`agent/session.go`) folds a delegated sub-agent's `InputTokens`/`OutputTokens`/`CacheRead`/`CacheWrite`/`CostUSD` into the parent session's own accumulated totals. Both `buildSessionTools`' `Subagent` executor and `buildFetchSummarizer` (`agent/agent.go`) now call it with the ephemeral sub-agent's `Stats()` right before closing it — including `Subagent`'s `background` mode, since that executor closure (and its `defer`) still runs to completion independently once the delegated work finishes, even after the launching turn has already ended.
- **The critical constraint, verified not just designed**: this must **never** influence `ContextUsage`/`ContextWindow` — those describe the parent's own current context occupancy, computed solely from its own most recent provider call, and a delegated sub-agent's tokens were never part of that call. `addDelegatedCost` only ever touches the reporting/billing fields, never `lastInputTokens` (the sole input to `ContextUsage`). Verified with a deliberately oversized (1,000,000-token) delegated delta producing zero change in `ContextUsage`.
- **Lock-free by necessity, not preference**: `addDelegatedCost` is called from *inside* a tool's executor goroutine, while `promptSync` holds `s.mu` for the entire turn (parallel tool execution included) — taking `s.mu` there would deadlock exactly like the bug `CurrentModel()` was fixed for (see the `subagent-timeout-background` project history, bug #3). Implemented with plain atomics (`atomic.Int64` for token counts; `CostUSD` stored as integer micro-dollars, since Go has no `atomic.Float64`) and drained into `s.stats` only from code that already holds `s.mu` — `updateStats` (every normal turn) and, for the case where a `background` `Subagent` call finishes *after* its launching turn already ended (nothing else would ever re-drain it), `Stats()` and `Meta()` themselves now drain-and-persist on read too.
- Tests (`agent/delegated_cost_test.go`): a deadlock regression test mirroring `TestCurrentModelDoesNotDeadlockUnderPromptSyncLock`'s exact pattern; the context-window-isolation guarantee; drain-via-`Meta()` for the background-finishes-late case; and two **live, end-to-end integration tests against a real connected provider** — a `Subagent`-using turn and a `Fetch`+`prompt`-using turn each confirmed to raise the parent session's `Stats().CostUSD`/`InputTokens` while `ContextUsage` stayed sane (not zero, not inflated). Full suite + `-race` + `go vet ./...` green.

## [0.76.53] - 2026-08-27

### Fix — a connection dropped mid-stream left a turn permanently stuck with no error, no retry, no signal to the user
- A `Bash` tool call was announced (spinner, `$ Bash …`) and then the turn simply never resolved — no result, no error, nothing — until typing a new prompt ("continua") started a fresh turn. Traced through the affected session's `.jsonl`: the stalled tool call left **zero trace** in the persisted history (not even an orphaned `tool_call` with no `tool_result` — the call never got far enough to be persisted at all), pointing at a connection drop mid-stream rather than a tool execution failure.
- **Root cause**: `internal/providers/llm/sse.go`'s `ParseSSE` closed its output channel identically whether the underlying reader hit a clean EOF or a genuine I/O error (`scanner.Err()` was never consulted or surfaced) — silently indistinguishable outcomes to any caller. Downstream, both `ParseAnthropicStream` (`anthropic.go`, used by `anthropic` + `claude-oauth`) and the OpenAI-dialect stream loop (`openai.go`'s `parseOpenAIStream`, used by `openai`/`minimax`/`opencode-go`/`ollama`/`ollama-cloud`) treated "the channel stopped yielding events" as proof the response was complete, without ever checking whether the stream's own natural end-of-turn marker (`message_stop` for Anthropic, `[DONE]` for OpenAI-dialect) had actually been seen. A connection reset mid-tool-call (e.g. while a `Bash` call's arguments were still streaming in) fell through both gaps at once: no error surfaced, and the function returned a "successful" response missing the tool call the model was mid-way through — the turn then looked complete with nothing left to run, so `EventTurnEnd` fired with the UI still showing the last event that DID arrive (`EventToolStart`) and nothing ever resolving it.
- **Fix**: `ParseSSE` now returns `(chan, errFn)` — `errFn()` reports the genuine underlying read error once the channel is drained, `nil` for a clean EOF, and (critically) also `nil` for a caller-initiated `ctx` cancellation (an `atomic.Bool` set by the watchdog goroutine *before* it closes the reader distinguishes "we closed this on purpose" from "it broke on its own" — the two outcomes look identical to `scanner.Err()`, so they can't be told apart without this flag). Both `ParseAnthropicStream` and `parseOpenAIStream` now track whether they actually saw the stream's natural end marker, and check `errFn()` after the loop — either failing condition now returns a genuine error instead of `resp, nil`. That error flows through the exact same path any other stream error already does (`runStream` → `promptSync` emits `EventError`) — **zero changes needed in `agent/session.go` or any transport**: it already checks `ctx.Err() != nil` *before* looking at the error's content, so a real user Stop() still correctly emits `EventStop` (not `EventError`) even though the stream functions now also return non-nil there. Fixing the two shared dialect parsers covers all 7 providers at once.
- Tests: `internal/providers/llm/sse_test.go` gained a genuine-read-error case (`errFn()` surfaces it) alongside the existing cancellation case (`errFn()` stays nil). `anthropic_stream_test.go` and `openai_test.go` each gained a "dropped connection mid-tool-call" regression test (reproducing the exact reported incident — a tool announced, then the connection breaks before its arguments finish streaming, now surfaces as an error) and a "clean EOF before the natural end marker" case. Full suite + `-race` + `go vet ./...` green; the OpenAI-dialect fix also closes a latent goroutine leak (`[DONE]`'s `break` used to abandon the still-running `ParseSSE` producer goroutine unconditionally instead of draining it).

## [0.76.52] - 2026-08-24

### Change — `Fetch`'s content-condensing sub-agent now knows when its input was truncated
- Follow-up to v0.76.51's `FetchSummarizer`. When the fetched body exceeds `fetchSummarizeMaxBytes` (200KB), the content handed to the summarizing sub-agent was silently cut — the sub-agent had no way to know its answer might be missing information beyond that point, and the calling agent (who never sees the raw body itself) had no signal at all that the condensed answer came from partial content.
...
- Models routinely sent a `prompt` argument to `Fetch` that our schema never declared and the tool silently ignored (`json.Unmarshal` drops unknown fields) — harmless, but wasted tokens on an instruction nobody read, every time it happened.
...
- Follow-up to v0.76.48. A subsequent edit (`"malformed tool arguments: Invalid JSON for this call — retry with corrected, valid JSON"`) was more concise than the original draft, but still didn't name the specific fix. Final wording: `"invalid JSON in tool-call arguments — retry this call with a single, well-formed JSON object"` — consistent lowercase, and names the actual constraint (ONE object, not concatenated fragments) matching the real observed failure mode. Tests and changelog updated to match this exact string.

## [0.76.48] - 2026-08-17

### Fix — malformed tool-call JSON from a model produced a misleading "missing required field" error instead of the real diagnosis
- A tool call failed with `missing required field: command` even though the model clearly intended to run a command — traced to the `_raw` wrapper the provider streaming layer (`internal/providers/llm/{anthropic,openai}.go`) uses when a model streams invalid JSON as tool-call arguments (e.g. concatenated fragments like `{"command":"a"}{"command":"b"}{"command":"c"}`, seen in practice with minimax-m3 and occasionally others). The wrapper (`{"_raw": "<the invalid JSON, escaped>"}`) is itself valid JSON, so it unmarshals successfully into any tool's typed input struct — `encoding/json` silently ignores unknown fields — leaving every declared field at its zero value. `requireFields` then reported the technically-true but misleading "missing required field", masking that the real problem was the model emitting unparseable JSON, not omitting a value.
- **Fix**: the wrapper is now detected centrally, before it ever reaches a tool's `Execute`/`ExecuteRich`.
  1. `types.RawWrapperKey` (`"_raw_"`, renamed from the bare `"_raw"` to make an accidental collision with a real tool field even less likely) and `types.IsRawWrapper(input)` — a new shared helper in the public, dependency-free `types` package (the one place both `agent/tools` and `internal/providers/llm` can depend on without `agent/` and `internal/providers/` importing each other, per the project's backend/frontend separation rule). Detection is structural, not just name-based: exactly one key, named `_raw_`, whose value is a string — so a legitimate tool with an unrelated single field is never misidentified.
  2. `agent/tools.Registry.Run` now checks `IsRawWrapper` before dispatching to any tool, for every tool (built-in, MCP, or SDK-custom) automatically — no per-tool code needed. On a match, it returns a clean, actionable, grep-able error: `"invalid JSON in tool-call arguments — retry this call with a single, well-formed JSON object"` — true to what happened, and (refined after an initial pass) names the specific fix — ONE object, not concatenated fragments — which is exactly the failure mode observed in practice (e.g. minimax-m3 streaming `{"command":"a"}{"command":"b"}`).
  3. No new logging dependency added to `agent/tools` (a public SDK package with none today) — the error already reaches every transport via the existing `EventToolResult{IsError: true}` event, and the message's fixed, grep-able prefix lets anyone who already logs (harness's own CLI, or an SDK consumer with its own `logx.Logger`) correlate how often this happens without any new plumbing.
- Tests: `types/tool_test.go` — `IsRawWrapper` across the wrapper itself, a normal call, an empty object, invalid JSON, a second key present, a non-string value, an object value, and a legitimately single-field tool call (all correctly `false` except the real wrapper). `agent/tools/registry_rawwrapper_test.go` — a wrapped input never reaches the tool's `Execute` and gets the new "invalid JSON in tool-call arguments" message naming the single-object fix (not the old "missing required field"); a normal call is completely unaffected. Full suite + `-race` + `go vet ./...` green.

## [0.76.47] - 2026-08-17

### Change — Slack directive's Rule 6 made deployment-agnostic and general-purpose
- Follow-up to v0.76.46: trimmed deployment-specific asides from the new "never assume who you're talking to" rule — the parenthetical `(e.g. whoever owns or usually operates this bot)` referenced this project's own operator by implication, which doesn't generalize to another tenant running this same transport with a different setup; also dropped redundant wording ("their own context, their own history, their own permissions") that repeated "answer for THAT specific person" without adding instruction. The verification example was briefly narrowed to a software-dev domain (`project, pull request, code`) and then widened back to `(a project, a task, a prior conversation, an admin action)` so the rule reads as genuinely general-purpose — Slack transport use isn't assumed to be coding-specific.
...
- The model was responding as if every Slack sender were the bot's usual operator — it never actually reasoned about *who* `<slack:user>` said was writing, especially in DMs, where there's no @mention rule forcing that check. The directive told the model how to use the `<slack:user>` tag mechanically (@mention in shared spaces, distinguish speakers in a channel) but never told it to resolve WHO that sender actually is and tailor the response to that specific person.
...
- Reported by a third-party dev embedding harness as a library: a `*agent.Agent` built with `AgentWithThinking("low")` (different from the operator's global `~/.harness/settings.json` `thinking_level: "high"`), when used to create a session via `POST /api/sessions` / `client.CreateSession`, ended up with `thinking: "high"` on the resulting session — the Agent's own explicit configuration was silently overridden, with no error or log anywhere.
...
- Raised after the OAuth refresh lock hardening: does `agent/schedule.Store` have the same exposure `credentials.json` had before it was hardened? Audited — yes, and structurally worse, because concurrent access here isn't a rare edge case, it's the documented, expected architecture: the agent ALWAYS opens the schedule store so `Schedule`/`ScheduleList`/`ScheduleDelete` work in any session, while `EnableScheduler`/`--scheduler` only decides whether THIS agent also runs the Engine that fires due prompts. Running TUI + Telegram + Slack + `serve` at once, each with its own in-process `*Store`, exactly one carrying `--scheduler`, is the intended deployment shape.
- **Two concrete bugs this caused**: (1) a lost write — any two processes read-modify-`os.WriteFile`d the WHOLE map with no lock, so the Engine's `RecordRun` (every 30s, in the `--scheduler` process) racing a `Schedule`/`ScheduleDelete` call from any other process's session silently discarded whichever wrote second; (2) a corrupted/lost audit trail — `RecordRun` persisted its entire possibly-stale in-memory map, so it could clobber an edit another process just made to a DIFFERENT schedule, with no `reloadIfStale`-equivalent to catch it.
- **Fix** (design: `docs/plans/2026-08-17-schedule-store-lock-hardening-design.md`), following the same pattern already validated for OAuth (`UpdateCredential`) rather than re-inventing one:
  1. `internal/config.acquireFileLock` (the UUID-ownership-guarded, 45s-stale-reclaim primitive hardened for OAuth) is exported as `AcquireFileLock` and reused directly by `agent/schedule` — no new package, no duplication; `agent/schedule` importing `internal/config` is the same cross-package pattern `agent/agent.go` already uses (the `internal/` rule only blocks OTHER modules, never sibling packages within harness).
  2. `Store` gained `reloadIfStale` (mirroring `CredentialsManager`'s) — `List()` now always sees the freshest disk state, so the Engine's every-tick read, the `ScheduleList` tool, and the server's read endpoint never serve a startup-time-stale snapshot.
  3. One atomic write entry point, `UpdateSchedule(owner, slug, fn)` — takes the file lock exactly once, reloads fresh state, and calls `fn(current, ok) → (next, action, err)` with a 2-value `UpdateAction` enum (`ActionNoop` | `ActionWrite`) deciding the outcome. `Set` and `RecordRun` are now thin wrappers over it (`RecordRun` reads the FRESH `cur` before incrementing — the fix for bug #2 — and no-ops instead of resurrecting a schedule another process deleted meanwhile). `Delete` stays a direct method (no callback): a delete never needs to inspect current state to decide anything, unlike `Set`/`RecordRun` — routing it through the callback would've meant a 3rd `ActionDelete` enum value neither of the others use, or a second bool return with a representable-but-invalid `write=true, delete=true` combination.
- Tests (`agent/schedule/lock_hardening_test.go`, `-race`): missing-schedule reporting, the `ActionNoop`/`ActionWrite` flag gating persistence, `fn`'s error aborting the write, a second `Store` instance seeing the freshest disk state a first one just wrote, **`RecordRun` applying its increment on top of a concurrent edit instead of clobbering it** (the exact bug #2 regression), `RecordRun` no-op'ing against a concurrently-deleted schedule, 10 concurrent `UpdateSchedule` callers never overlapping in the critical section, and `Delete` racing a concurrent `Set` resolving without corruption. All existing `agent/schedule` tests pass unchanged. Full monorepo suite green under `-race`; `go vet ./...` clean.
...
- **Reported** by a third-party dev embedding harness as a library (a long-running process hosting multiple concurrent sessions, each with its own real working directory): the built-in tools operated against the hosting OS process's real working directory, not the session's logical `CWD` — confirmed by inspection (zero `cmd.Dir =`/`filepath.Join(cwd, ...)` in `bash.go`/`file.go`/`edit.go`), in contrast to `MemoWrite`/`MemoSearch`/`MemoDelete` (`agent/tools/memory.go`), which already receive `cwd` explicitly and partition by it. `Session.CWD` reached only the system prompt (`## Working Directory\n\n<cwd>`, purely informational) — never applied by code. Invisible until now because every official harness transport (CLI/TUI/Telegram/Slack/ACP/server) calls `os.Getwd()` once per process and treats that as "the" cwd, so `Session.CWD == os.Getwd()` always held — not a documented 1-process-1-session assumption, just a gap nobody had exercised. No measurable performance impact from fixing it (a struct field assignment and a cheap `filepath.IsAbs` check per call).
- **Fix — two layers** (design: `docs/plans/2026-08-14-tool-session-cwd-design.md`):
  1. `Bash`, `ReadFile`, `WriteFile`, `Edit` (`agent/tools/{bash,file,edit}.go`) now take `cwd string` in their constructor, same pattern as `MemoWrite`. A new shared `resolvePath(cwd, path)` (`agent/tools/path.go`) resolves a relative path against `cwd` and returns an absolute path as-is (no sandboxing — same trust model as before, just correct relative resolution). `Bash` sets `cmd.Dir = cwd` on BOTH the foreground `exec.Command` and `runBashBackground`'s detached one — covering the background path was necessary, not just the common case.
  2. Since these 4 tools need a PER-SESSION cwd but used to be registered once at the AGENT level (`agent.defaultTools()`, shared by every session), they moved into `buildSessionTools` — built fresh per session, with that session's own `cwd`. `defaultTools()` now only seeds `Fetch` (no cwd dependency). No external SDK consumers called these constructors directly, so this isn't a breaking change for anyone but harness's own wiring.
  3. System prompt (`buildSystemPrompt` in `agent.go`) gained an explicit instruction alongside the `## Working Directory` line: prefer absolute paths for file tools and Bash commands touching specific files. This reinforces — rather than replaces — the code fix: previously the model followed this convention only by learned habit, with nothing in the prompt actually asking for it.
- Tests (`agent/tools/cwd_test.go`): `resolvePath` unit cases; a relative Write/Read/Edit lands inside the session cwd, not the process's real one; absolute paths are unaffected regardless of the cwd passed in; **the exact reported regression** — two tool sets built with different cwds (simulating two concurrent sessions in one process) never cross-contaminate on a relative path; Bash foreground AND background both run with `cmd.Dir` set to the session cwd (verified via `pwd`). Existing tests that called the old zero-arg constructors updated. Full suite + `-race` + `go vet ./...` green.
...
- **Symptom**: running several harness processes on one machine (multiple TUI windows + Telegram + Slack + serve, all against the same `~/.harness/`) intermittently produced a permanent `refresh HTTP 400 … invalid_grant … Refresh token not found or invalid` on claude-oauth, in the middle of active use — recoverable only via `harness connect claude-oauth`.
- **Root cause**: Anthropic OAuth refresh tokens are single-use with rotation, so redeeming the same token twice permanently invalidates it. The cross-process file lock that serializes the refresh reclaimed any lock older than **5s** as "abandoned" — but a legitimate refresh could hold it LONGER: `refresh()` used `http.DefaultClient` (infinite timeout), tried **2 endpoints in series**, and ran its full 3-attempt `1s→2s→4s` backoff **inside** the lock. Under slow network, Process A held the lock >5s → Process B judged it stale, removed it, entered → both redeemed the same token → one won, the other got permanent `invalid_grant`. A's unconditional `release()` then deleted B's lock, cascading. (The prior v0.76.11 `reloadIfStale` fix closed cross-process READ staleness but not this premature lock reclaim.)
- **Fix — defense in depth (design doc: `docs/plans/2026-08-13-oauth-refresh-lock-hardening-design.md`)**:
  1. **Bounded, single-endpoint refresh** (`claude_oauth.go`): dropped the 2-endpoint fallback (both were verified live+identical, so it bought no resilience but exposed a double-redeem vector if the first endpoint redeemed then its response was lost); refresh now hits only `platform.claude.com` via a dedicated `http.Client{Timeout: 30s}` instead of the infinite default.
  2. **Retry/backoff moved OUTSIDE the lock**: the lock is now taken and released once per attempt, so its hold time is one bounded refresh (≤30s), never the multi-second backoff sleeps. The re-check under the lock (use a fresh disk token if a peer already refreshed) now runs every attempt — the key double-redeem safety net. The fast path (valid token) still never touches the lock.
  3. **Lock ownership guard** (`filelock.go`): each lockfile carries a unique UUID; both `release` and stale-reclaim remove it only if it still holds the expected token (compare-then-remove), so a reclaimed slow holder can never delete the reclaimer's lock — the cascade is gone.
  4. **Login flow (`internal/oauthflow/`) collapsed to the same single endpoint**: `claude.go`'s `claudeTokenURLs` list → one `claudeTokenURL` const, and `postToken` simplified from a multi-endpoint loop to a single `endpoint string`. The initial-login authorization-code exchange had the same 2-endpoint fallback with the same single-use double-redeem vector (redeem then lose the response → fallback redeems a consumed code → login fails). Less severe than the refresh path (a failed login just means retry `harness connect`, no saved creds left broken), but collapsed for consistency and to remove the dead fallback.
  - **Threshold realignment**: `staleFileLockAge` **5s → 45s** (30s bounded refresh + margin), so an alive-but-slow holder is never mistaken for a crash.
- Tests: `internal/config/filelock_test.go` (`-race`) — normal release removes own lock; **a reclaimed holder's release does NOT stomp the reclaimer's lock** (the exact cascade regression); only stale locks are reclaimed; 20-goroutine contention with an atomic counter asserting never >1 in the critical section. `internal/providers/claude_oauth_test.go` — `isAuthError` retry-vs-abort classification (invalid_grant/400/401/403/token_expired/revoked permanent; timeout/500/429 retryable) and the single-bounded-endpoint invariants. Full `-race` suites + `go vet ./...` green.
...
- **New package `internal/oauthflow`** — designed for extensibility: one `OauthFlow` interface (`Start()` → generate PKCE, open browser, return the auth URL; `Exchange(code)` → swap the pasted code for credentials), with the pieces every provider reuses (PKCE per RFC 7636, browser open, the generic token POST) in `oauthflow.go`, and each provider's specifics in its own file. `claude.go` holds `NewClaudeOauthFlow()` (returns the interface; the concrete `claudeOauthFlow` is unexported) with Claude's `client_id`/scopes/endpoints and the `#state`-fragment strip. `oauthflow.For(provider)` is the ONE place mapping a provider name to its flow — so callers that only know the name at runtime (the CLI's `case "oauth"` connect branch, the TUI's `cmdConnect`) stay provider-agnostic and never grow their own switch. Adding a future provider (Codex, etc.) is one new file implementing `OauthFlow` plus one `For` case — no call-site changes. Lives at `internal/oauthflow` (not under `providers/`) because an OAuth flow isn't a provider — it's auth infrastructure providers consume.
- The two-phase split (`Start` then `Exchange`) lets both transports drive one shared implementation with their own input model: the CLI blocks, the TUI uses value-capture.
- **CLI** (`internal/cli/oauth.go`): `RunOAuth(provider)` resolves the flow via `oauthflow.For` (provider-agnostic — the `case "oauth"` branch passes the provider name, doesn't assume Claude) and blocks on a new `PromptLine` (visible echo, unlike `PromptSecret`) for the code paste; returns a clean error on a non-TTY instead of blocking.
- **TUI** (`internal/tui/commands.go`, `tui.go`): `cmdConnect` for a subscription provider opens the browser and drops into the EXISTING value-capture (`pendingValue.oauthFlow oauthflow.OauthFlow`), completing via `Exchange` on the next submit — no subprocess, no leaving raw mode. This corrects the old assumption (in the now-removed keychain path) that OAuth login was incompatible with the TUI's raw mode: that only applied to spawning `claude auth login` as a subprocess, never to opening a browser + capturing a pasted value (which is exactly what the API-key path already does).
- **Removed the keychain/credentials-file read** (`ObtainOAuthCredentials` and its `readClaudeCredentials*` helpers, and the whole `internal/providers/authflow` package) — kept commented out at the top of `oauthflow/claude.go` as a revert-fast fallback during this trial phase, to be deleted once the native flow is proven in the field.
- Updated the re-auth error message in `internal/providers/claude_oauth.go` (shown when a token refresh fails): now says `run 'harness connect claude-oauth' to re-authenticate` instead of the stale `run 'claude auth login'` — harness no longer depends on Claude Code, so it points at its own native connect command.
- New tests: `internal/oauthflow/oauthflow_test.go` (shared PKCE S256 validity + uniqueness, `randomURLSafe` length/alphabet) and `claude_test.go` (auth URL params exact + matching the stored verifier, `NewClaudeOauthFlow` returns the interface, `Exchange` strips the `#state` fragment / rejects empty codes / rejects being called before `Start`). `openBrowser` is a swappable package var so tests stub it — previously every `go test` run actually launched a browser. Validated end-to-end against a real account: CLI connect, TUI `/connect`, and a real agent turn (`claude-oauth/claude-sonnet-5` → replied) all succeeded. Full suite + `-race` green.

## [0.76.19] - 2026-08-05

### Fix — context overflow: auto-compact fired too late, and compaction itself could overflow (`agent/session.go`)
- A session at 99.5% usage showed TWO consecutive compaction attempts both failing with `prompt is too long: 1,012,151 tokens > 1,000,000 maximum` (and `1,028,692` on the retry) — the rescue mechanism dying of the exact disease it exists to cure. A 1M-window model session.
...
- **Fixes**:
  ...
  3. **Emergency prune** in `generateCompactionSummary`: if the history to summarize already exceeds a character budget (`compactionCharBudget`), whole turns are dropped oldest-first (`pruneOldestTurns`) until it fits, with a notice injected recording how many turns were omitted so the summary doesn't feign continuity it no longer has. Turns are the **atomic unit** (`turnStarts`/`isToolResultOnly`): dropping is only ever done on turn boundaries — a real user prompt, never a tool-result-only user message — so a `tool_call` and its matching `tool_result` are never separated (a shape every provider rejects). The last turn is never dropped (indivisible minimum); the extreme case of a single turn larger than the whole budget is deliberately left to fail rather than split a turn or truncate tool output — a lot of machinery for a case that shouldn't occur on a real large-window model (explicit design decision).
     - **The budget targets 0.95 of the window (the same `autoCompactThreshold`) and is CALIBRATED to the session's real chars-per-token ratio**, not a fixed guess. `pruneOldestTurns`/`approxSize` work in characters, but the window is in tokens, so the token budget is converted to chars via `approxSize(history) / lastInputTokens` — the actual ratio this session exhibits (the provider's real token count for the same history we measured in chars). A real field study of a stuck session (99.5% full, 2,226,183 chars = 995,087 tokens) measured **2.237 chars/token** — where the original fixed `window*4/2` budget (2M chars, ~50% of the window) would have thrown away far more context than necessary, and a naive fixed-4 budget at 95% (3.8M chars) would have **exceeded the real history and pruned nothing — re-overflowing**. The calibrated budget (≈2.125M chars) prunes just 6 of 88 turns, keeping ~94% of the context. Falls back to a fixed 4 chars/token only when there's no real measurement yet (`lastInputTokens == 0`, e.g. a resumed session before its first turn).
...
- **Reported issue**: resuming a session showed `82.7%/1.0M`, then after the first turn the footer read `0.0%/1.0M` — as if the context had emptied itself.
- **Root cause**: `s.lastInputTokens` (the single source both the TUI gauge and ACP's `usage_update` derive from) summed only `InputTokens + CacheRead`, omitting `CacheWrite`. All three were sent to the model as context on that request — they're just billed differently, and Anthropic's own field name says so outright: `cache_creation_input_tokens` is INPUT that was additionally written to the cache. On a turn that (re)writes the cache — the norm right after a compaction, a model switch, or any system-prompt change — nearly the whole context is reported as cache creation. The reported turn was 2 fresh + 0 read + 827.6k written on a 1M window: the old formula produced 0.0002% (rounding to `0.0%`) while true occupancy was 82.8%, matching the 82.7% persisted from the turn before — proving the context had not shrunk at all, the accounting had lost it.
- **This was functional, not cosmetic**: `ContextUsage >= 0.98` is what triggers the mid-turn auto-compact, so under-counting could leave that guard silent while the real context ran to the window limit, losing the turn to a provider overflow instead of compacting in time. A test demonstrates the difference on a nearly-full cache-write turn: `0.0010%` (auto-compact silent) → `98.5%` (auto-compact fires).
- **Fixed for both clients at once**, since both derive from the same value: the TUI gauge (which receives the pre-computed ratio) and ACP's `usage_update{used,size}` (which receives raw tokens, per the ACP spec — `used`/`size` are token counts, there is no ratio field in the protocol).

### Fix — footer session totals collapsed to per-turn values on the first event after resume
- **Reported issue**: on resume the footer showed `↑112.6M ↓2.5M R2141.0M W62.2M $1099.055`; after the first turn it became `↑2 ↓2.5M R0 W827.6k $1101.125` — three of six stats collapsed.
- **Root cause**: two semantics fought over the same fields. `loadStatsFromSession` loaded the persisted **session totals** on resume, then the first `tokens` event overwrote `input`/`cacheRead`/`cacheWrite` with that turn's **per-turn** values. `↓` and `$` were spared only because they already read accumulated fields (`TotalOutput`/`CostUSD`). The agent did compute `TotalCacheRead`/`TotalCacheWrite`, but `server/sse.go` never serialized them and `client.Event` had no fields for them, so the TUI had no accumulated values available to read — and `TotalInput` didn't exist at all, despite `SessionStats.InputTokens` being persisted.
- **Fix** — `types.TokenUsage` restructured so every field has exactly ONE semantic and at least one real consumer, reusing the wire names clients already consume rather than adding parallel `total_*` variants: **live context** (`input`, `context_usage`, `context_window` — shrink after a compaction) vs **session history** (`total_input` ← new, `total_output`, `cache_read`, `cache_write`, `cost_usd` — only grow, mirror the persisted `SessionStats` 1:1). Removed three dead fields nobody could consume: `Output` (emitted, never read by any client), `TotalCacheRead` and `TotalCacheWrite` (computed and emitted in Go but never serialized, now redundant with the reused `cache_read`/`cache_write`). **Net: +1 field, −3.** `SessionStats` and the on-disk format are **untouched** — they were already correct; only the event needed to mirror them.
- **`compact_end` now refreshes the gauge immediately**: `compact()` emits a follow-up `EventTokens` with the live context zeroed and history intact, instead of leaving clients showing the pre-compaction figure until the next turn happened to emit one. Reuses `EventTokens` deliberately rather than adding stats fields to `compact_end` — every client already handles it.
- **Removed `omitempty` from all tokens fields in `client.Event`**: the struct's own comment stated zeroes are meaningful, but the tags dropped them on re-encode — making "0 cache reads" or "0.0% context right after a compaction" indistinguishable from "not reported". The CLI's `json`/`json-stream` modes re-serialize events, so this was a real data-loss path. Same class of bug as the `loop` field in 0.76.7.
- New tests: `agent/token_stats_test.go` (the exact field-reported numbers, all five cache-mode shapes, and the auto-compact threshold consequence), `server/sse_test.go`'s `TestFormatEventTokensCarriesBothSemantics` + `TestFormatEventTokensSendsZeroesExplicitly`, `client/stream_test.go`'s `TestEventTokensZeroesSurviveReEncode`, `internal/tui/footer_stats_test.go`'s `TestFooterTotalsSurviveTheFirstEventAfterResume` + `TestFooterGaugeDropsToZeroOnCompaction`. Verified in both directions: reverting the TUI to read `evt.Input` reproduces the reported collapse (`got 827602, must be >= 112626989`). Wire contract additionally validated end-to-end against the compiled SDK. Full suite + `-race` green.
...
- **Reported issue**: the TUI showed `⚠ reached the 120-iteration limit — summarizing progress` and then NOTHING — no summary, no error. Intermittent: the same session had produced the summary correctly earlier the same day.
- **Forensic evidence (not hypothesis)**: inspected the reported session's on-disk JSONL directly. Of 8 historical max-iteration events, 4 are followed by an `assistant` message (summary produced, user saw it) and 4 are followed immediately by the user's next prompt — no assistant message at all. Since `requestProgressUpdate` persists the summary to the store BEFORE any transport renders it, the absence on disk proves the summary was never generated: the loss was in `agent/`, not in SSE, the client, or the TUI.
- **Root cause #1 — dead code in `drainFollowUps`**: `cancel()` (the turn's unconditional resource cleanup) ran on the line BEFORE `if err != nil && ctx.Err() == nil { s.emit(errorEvent(err)) }`. Reading `ctx.Err()` after our own `cancel()` always reports `context.Canceled`, indistinguishable from a user `Stop()` — so that condition was never true and the emit was **dead code that never executed once**. This silently broke a contract two call sites explicitly documented relying on it ("propagated up so drainFollowUps emits an EventError"): `requestProgressUpdate`'s failure path AND its empty-summary guard. Errors raised INSIDE the ReAct for-loop were unaffected (they emit their own `EventError` before returning), which is exactly why only this path went silent. Fixed by capturing `wasCancelled := ctx.Err() != nil` BEFORE `cancel()`.
- **Root cause #2 — no retry on the summary call**: `requestProgressUpdate` makes a single, unrepeatable LLM call at the end of a ~120-iteration turn with NO retry, unlike `generateCompactionSummary` which already had 3 attempts with exponential backoff. A transient failure (OAuth token refresh landing mid-call, a 429, a dropped connection) permanently cost the user the only report of all that work. Now retries 3× with backoff, aborting immediately on a cancelled ctx (retrying can't succeed and would just burn the backoff). Safe despite this call streaming to transports: a failed attempt emits at most a partial text run and transports append deltas into the live block, so a retry continues in the same block — losing the summary entirely is strictly worse than a rare duplicated fragment.
- **Also fixed — the reserved iteration had no Stop symmetry**: the `for` loop guards `ctx.Err() != nil` at the top of every iteration (emitting `EventStop`), but the reserved max-iterations block lives OUTSIDE the loop and had no such guard — a Stop pressed right at the cap produced neither summary, nor error, nor `EventStop`. It now has both an entry guard and a mid-summary cancellation check, mirroring the loop exactly. **User Stop semantics deliberately unchanged**: a Stop stops everything, summary included (per explicit product decision — the alternative of decoupling the summary's ctx from Stop was considered and rejected).
- **Design note**: moving `requestProgressUpdate` INTO the `for` loop (as `i == maxIterations-1`) was considered and rejected — the loop's own entry guard would short-circuit before the summary ever ran, cementing the exact bug being fixed, and the reserved iteration differs from a normal one on 6 axes (no tools, injects a system-generated prompt, can't process tool results, no auto-compact, always terminal, validates empty text), which would have required ~5 `if isLastReserved` branches inside a body that already has 7 exit paths.
- New tests: `agent/drain_error_test.go`'s `TestCancelBeforeErrCheckMakesEmitDeadCode` (pins the before/after-`cancel()` ordering semantics the fix depends on) and `TestErrorReportingDecision` (decision table so a future change to that condition can't silently reintroduce the swallowing). Verified the test proves the bug in BOTH directions: temporarily reverting to the broken ordering confirmed a real summary error is suppressed and the emit unreachable. Full suite + `-race` green.

## [0.76.12] - 2026-08-04

### Add — dependency-free presence validation for built-in tool inputs (`agent/tools/validate.go`)
- **Audit finding**: across the 15 built-in tools, several `"required"` JSON Schema fields had NO actual presence check in Go — they either failed only by ACCIDENT deep in an OS/network call (`Bash.command`, `Read.path`, `Write.path`, `Edit.path`, `Fetch.url` — e.g. an empty `command` runs `bash -c ""` successfully with no error at all), or fell through to a response that reads exactly like a normal, successful outcome instead of flagging bad input (`Skill.name`, `MemoDelete.slug`, `ScheduleDelete.slug` — e.g. `slug=""` on `MemoDelete` returns "No memory found with slug \"\"", indistinguishable from a legitimately-absent slug).
- Considered `go-playground/validator/v10` (~985KB binary size increase measured directly, plus a NEW transitive dependency on `golang.org/x/crypto` harness didn't have before) — rejected in favor of a **dependency-free** alternative per Golden Rule #1 ("no new dependencies without approval — solve with stdlib first"): our actual need is presence checking only (no emails/URLs/cross-field rules), which `reflect.Value.IsZero()` already covers completely.
- **New**: `agent/tools/validate.go`'s `requireFields(v any) error` — reflection over a struct's `validate:"required"` tags, reporting every field still at its Go zero-value using its `json:"name"` tag (what the model actually sent) in the error, never the Go field name. Called immediately after `json.Unmarshal` in every tool's `Execute`/`ExecuteRich`, before any business logic runs. Zero bytes added to the binary (no new import — `reflect` is already stdlib), zero `go.mod`/`go.sum` changes.
- Tagged: `Bash.command`, `Read.path`, `Write.path` (NOT `.content` — an empty file is a legitimate write), `Edit.path` (NOT `old_text`/`new_text`/`edits[]` — their XOR structural rule stays exactly where it already lived, in Go, right below), `Fetch.url` (NOT `body`/`json`/`form`/`files` — same reasoning, their mutual-exclusion rule is business logic, not presence), `Skill.name`, `MemoDelete.slug`, `ScheduleDelete.slug`, `MemoWrite.{slug,content}`, `Schedule.{slug,cron,prompt}` (their own manual `if x == ""` checks replaced by the shared helper for a consistent message — `Schedule`'s underlying store still independently validates the cron expression itself; that stays untouched). `Subagent.prompt` and `ColleagueAsk.{colleague,prompt}` gained the tag ALONGSIDE their existing `strings.TrimSpace()` checks (kept, not replaced) — `IsZero()` only catches "absent", not "all-whitespace", which those two already guarded against.
- New tests: `agent/tools/validate_test.go` (`requireFields` in isolation — value vs pointer, JSON name in the error, only tagged fields checked). `agent/tools/validate_migration_test.go`'s `TestRequiredFieldPresenceIsEnforced` (regression test covering all 13 migrated fields at once), `TestWriteContentEmptyIsStillLegitimate` (locks in the one deliberate exception), `TestSubagentAndColleagueAskStillCatchWhitespaceOnly` (confirms the TrimSpace guards weren't dropped). Full suite + `-race` green; `go.mod`/`go.sum` diff confirmed empty.

## [0.76.11] - 2026-08-03

### Fix — multiple harness instances didn't see credentials.json/settings.json changes made by another process
- **Reported issue**: with several harness instances (TUI windows, Telegram, Slack, serve) running at once against the same `~/.harness/` files, an OAuth refresh-token failure in one instance required running `/connect` in EVERY instance individually (or restarting each) — even though a single `/connect` already persisted a valid new token to `credentials.json`. Same class of staleness applied to `settings.json` (`/model`, `/mcp add`, etc. in one instance were invisible to every other running instance).
- **Root cause** (`internal/config/credentials.go`, `internal/config/settings.go`): both `CredentialsManager` and `SettingsManager` are per-process singletons that load their on-disk file exactly ONCE at startup (or whenever THAT process itself writes). Every read method (`Credential`, `APIKey`, `OAuth`, `ActiveModel`, `ThinkingLevel`, `Provider(s)`, `MCPServer(s)`) served straight from that in-memory snapshot forever — a write from a DIFFERENT process was invisible until the reading process restarted. `claude_oauth.go`'s `syncFromDisk()` — named as if it re-read the file — actually called straight into this same stale in-memory cache, so its own doc comment's claim ("every other instance picks up new creds on its very next call") was never actually true.
- **Fix** — both managers gained a `loadedAt time.Time` field (the file's mtime as of the last real `load()`) and a `reloadIfStale()` check, called at the top of every public read method: a cheap `os.Stat` compares the file's current mtime against `loadedAt`; only when the file actually changed does a full `load()` (the `os.ReadFile`+`json.Unmarshal`) run. No polling, no background goroutine — purely reactive to being read.
- **Also fixed in passing**: `settings.json` had NO cross-process write lock at all (unlike `credentials.json`, which already had one for its single-use OAuth refresh tokens) — two instances writing at the same time (e.g. `/model` in one, `/mcp add` in another) could silently clobber each other. `internal/config/filelock.go` extracts the lock primitive (`os.OpenFile(O_CREATE|O_EXCL)`, stale-lock reclaim after 5s) out of `credentials.go` into a shared file, and every `SettingsManager` write method (`SetActiveModel`, `SetThinkingLevel`, `SetProvider`, `DeleteProvider`, `SetMCPServer`, `DeleteMCPServer`) now takes it and re-reads the latest disk state before applying its change — the same read-modify-write discipline `SetCredential`/`DeleteCredential` already had.
- New tests: `internal/config/credentials_test.go`'s `TestCredentialCrossProcessReload`, `internal/config/settings_test.go`'s `TestSettingsCrossProcessReload` — both spin up two independent manager instances against the same file, write through one, and assert the other picks it up on its next read without being recreated.
- Verified manually against the compiled binary: started a real `harness serve` process, changed `thinking_level` via a SEPARATE `harness settings set` process, and confirmed `GET /api/settings` on the still-running server process reflected the new value immediately — no restart. Full suite + `-race` green.
...
- **Gap**: `types.Event.Loop` (the 0-based iteration index `agent/session.go`'s `promptSync` already tracks internally) was never included in the SSE wire payload — `server/sse.go`'s `formatEvent` emitted a bare `{"type":"loop_start"}`/`{"type":"loop_end"}` with no index at all. Every consumer that wanted a per-turn iteration count had to reconstruct one client-side by hand — which is exactly what the TUI did (`currTurn++` on every `loop_start`), and that hand-rolled counter is what silently never advanced for the reserved max-iterations summary iteration fixed in 0.76.6 (that exit path's `loop_start` existed in the event stream but the TUI's own increment logic had no special knowledge that it should count too — it should have, and now does, simply by trusting the number the agent already sends).
- **Fix**:
  - `server/sse.go` — `formatEvent` now includes `"loop": e.Loop` on both `loop_start` and `loop_end` (the SAME value for a given iteration's matched open/close pair, not two separate counters). No `omitempty`-equivalent skipping: iteration 0 is a real, common value, and dropping it would make it indistinguishable from "field absent."
  - `client/event.go` — new `Loop int` field, `json:"loop"` (no `omitempty`, same reasoning).
  - `internal/tui/events.go` — the `loop_start` case now sets `t.currTurn = evt.Loop + 1` (the `+1` is only for this 1-based footer display) instead of incrementing a hand-maintained counter — trusting the agent's own number instead of reconstructing it. `turn_start` no longer resets `currTurn` to 0 (unnecessary now: every `loop_start` sets the value outright, never adds to whatever was there before).
- New tests: `server/sse_test.go`'s `TestFormatEventIncludesLoopIndexOnLoopStartAndEnd` (both event types carry the right index, including large indices), `TestFormatEventLoopZeroIsNotOmitted` (the specific zero-value pitfall); `client/stream_test.go`'s `TestStreamEventsDecodesLoopIndexIncludingZero` (a real SSE round trip decodes `Loop` correctly, including 0); `internal/tui/events_test.go`'s `TestTurnCounterSetFromLoopStartsOwnIndexNotAStaleCarry` (a stale prior `currTurn` value never leaks into a fresh assignment) and an updated `TestTurnCounterIncrementsForReservedMaxIterationsLoop` (now asserting the exact `Loop` values `session.go` actually sends: `0` then `maxIterations-1`). Verified manually against the compiled binary: `harness -p "..." --output json-stream` now shows `{"loop":0,"type":"loop_start"}` / `{"loop":0,"type":"loop_end"}` on the wire. Full suite + `-race` green.

## [0.76.6] - 2026-08-03

### Fix — TUI spinner (and footer iteration counter) froze at the "reached the N-iteration limit" warning, hiding the summary that streamed in right behind it
- **Root cause** (`agent/session.go`) — the reserved "summarize progress" iteration (the one `for i := range s.maxIterations - 1` sets aside) ran `requestProgressUpdate`'s LLM call as a gap between the last real iteration's `loop_end` and `turn_end`, with a bare, unbalanced `EventLoopEnd` emitted right before `EventMaxIterationsReached` — no `EventLoopStart` of its own, unlike every other iteration in the loop. This wasn't just a TUI cosmetic gap: it meant any client behavior keyed specifically on `loop_start` never fired for this iteration.
- **Concrete symptom** — the TUI's spinner is re-armed on `loop_start` (added in `a7b80a2` to cover a mid-turn auto-compact turning it off in `compact_end`). If the auto-compact right before hitting the iteration cap (`ContextUsage >= 0.98`, checked between iterations) happened to be the LAST one before the cap, `compact_end` turned the spinner off and — because this exit path had no `loop_start` — nothing turned it back on. The turn looked completely frozen right at the "⚠ reached the N-iteration limit — summarizing progress" warning, even though the summary was still being generated and about to stream in as ordinary `text`/`text_end` events. The footer's "(turn/max_iterations)" counter (incremented only on `loop_start`) had the same gap, silently under-reporting the true iteration count.
- **Fix** (`agent/session.go`) — the reserved iteration now emits its own `EventLoopStart`/`EventLoopEnd` pair, exactly like any other iteration: `LoopStart` right before `EventMaxIterationsReached` (so the TUI's spinner and counter both react to it, same as any mid-turn iteration), and the matching `LoopEnd` after `requestProgressUpdate` returns. The `internal/tui` handling of `max_iterations_reached` itself needed no change — with `loop_start`/`loop_end` symmetry restored, the existing `loop_start` case already re-arms the spinner and increments the counter; a `WARN` message alone doesn't correlate with the spinner freezing, only the loop-start gap did.
- **Investigation also confirmed the summary text itself was never being swallowed** — a dedicated test proved the TUI's `"text"` event handling has zero dependency on spinner state; if `requestProgressUpdate`'s response streamed in via SSE, it was always rendered. The frozen-spinner illusion (not a text-rendering bug) was the full explanation for "the summary never showed up."
- New tests: `internal/tui/events_test.go`'s `TestSpinnerStaysOnAfterMaxIterationsReachedFollowingMidTurnCompact` (spinner re-arms via the reserved iteration's `loop_start`), `TestSummaryTextRendersAfterMaxIterationsReached` (the summary text reaches history regardless), `TestTurnCounterIncrementsForReservedMaxIterationsLoop` (the footer counter advances for the reserved iteration too). Full suite + `-race` green; verified the fix doesn't affect `transports/{telegram,slack,acp}` (none of them attach behavior to `loop_start`/`loop_end`).
...
- **New public package `logx/`** — just the contract, nothing else: `Logger` interface (`Info`/`Warn`/`Error(component, event string, kv ...any)`) and `NewNilLogger()` — a constructor returning a `Logger` that discards everything, not an exported struct. Any SDK consumer can implement `Logger` to route harness's backend logs anywhere (stdout, a file, a remote collector), without needing (or being able to touch) harness's own concrete implementation. Both this package's `nilLogger` and `internal/logx`'s `harnessLogger` are unexported — constructed only through `NewNilLogger()`/`NewHarnessLogger()` respectively, so every caller depends on the `Logger` interface, never a concrete (and, being stateless, otherwise trivially "reimplementable but shouldn't be") type — more idiomatic than exporting empty structs directly.
- **`internal/logx` refactored** from package-level functions (`logx.Info(...)`) to `NewHarnessLogger()` — a constructor for harness's own `Logger` implementation, with a compile-time assertion (`var _ logx.Logger = harnessLogger{}`). Same line-oriented output format as before (`LEVEL [component] event k=v`), zero behavior change — only internal/cli ever calls `NewHarnessLogger()`, passing the result explicitly into every `server.Run`/`transports/telegram.Run`/`transports/slack.Run` call the real `harness` binary makes.
- **`server`**: `ServerOptions.Verbose bool` replaced with `ServerOptions.Logger logx.Logger` (defaults to `logx.NewNilLogger()` when nil); new `server.WithLogger(l)` Option, default `logx.NewNilLogger()`. `requestLogger` middleware is now ALWAYS registered (no longer gated by a verbose conditional) — with the default nil logger it's simply a no-op call per request, which is what let the `Verbose` boolean disappear entirely in favor of "always run the middleware, inject silence when nothing should be logged."
- **`transports/telegram`, `transports/slack`**: both gain `WithLogger(l logx.Logger)` (default `logx.NewNilLogger()`) and a `Transport.logger` field. ALL ~40 existing `logx.Info/Warn/Error` call sites across both packages (`telegram.go`/`commands.go`/`files.go`/`pump.go`/`upload.go`; `slack.go`/`files.go`/`pump.go`/`upload.go`) migrated to `t.logger.Info/Warn/Error` — neither package imports `internal/logx` anymore. Each transport's own in-process `server.Server` always gets `logx.NewNilLogger()` explicitly (never this transport's own injected logger), so a request never gets logged twice by two different layers.
- **`transports/acp`**: deliberately has NO public `WithLogger` — it never logs anything itself (pure JSON-RPC protocol translation over stdin/stdout), so there's nothing for a caller to configure. Its in-process server always gets `logx.NewNilLogger()` internally, unconditionally.
- **`internal/tui` and `internal/cli/server.go`** (the CLI's own one-shot `-p` in-process server): both now pass `server.ServerOptions{Logger: logx.NewNilLogger(), ...}` explicitly, replacing the old `Verbose: false` — same silent behavior, expressed as an injected no-op logger instead of a boolean. This was the actual motivating case for the whole feature: the TUI shares stdout/stderr with the raw-mode terminal renderer, so it could never safely turn `Verbose: true` on for its own server; now that's just "don't pass a real logger," the same mechanism every other silent caller uses, instead of a boolean that only ever meant "must stay off forever here."
- **`internal/cli/kong_run_{serve,telegram,slack}.go`**: all three now pass `internal/logx.NewHarnessLogger()` explicitly via the new `WithLogger` option — `harness serve`/`harness telegram`/`harness slack` log exactly as before this change. `kong_run_acp.go` and `kong_run_tui.go` are unchanged (no `WithLogger` to wire — ACP never logs, and the TUI's in-process server was already covered above).
- **`harness.go` facade**: `Logger` re-exported as a type alias of `logx.Logger`; `NewNilLogger` re-exported as a `var` alias of `logx.NewNilLogger` (a function, not a type, since the underlying concrete type is unexported); `ServerWithLogger`/`TelegramWithLogger`/`SlackWithLogger` added (direct aliases, same pattern as every other `XxxWith*`); `ServerWithVerbose` removed (superseded by `ServerWithLogger`). `RunAcp`'s doc comment explicitly notes it has no `AcpWithLogger`.
- New tests: `logx/logx_test.go` (`NewNilLogger()`'s result behaves correctly — no panic, no observable effect; can't assert on the concrete type since it's unexported, by design), `internal/logx/logx_test.go` (updated to call `NewHarnessLogger()`'s result's methods — same format assertions as before), `server/proxy_test.go` (renamed `TestBroadcastSilentWithNilLogger`/`TestBroadcastLogsWithRealLogger`, now injecting a `logx.Logger` instead of flipping a bool), `server/run_test.go` (`TestRunDefaultsToNilLoggerSilently`, `TestRunUsesInjectedLoggerForRequests` — a real HTTP request through a `Run`-started server reaching an injected recording logger), `transports/telegram/logger_test.go` and `transports/slack/logger_test.go` (`WithLogger` correctly populates `Options.logger`, defaults to nil before `Run` applies its own default).
- Verified manually against the compiled binary: `harness serve` still logs the exact same lines as before (`listening`/`instance`/`request`); `harness acp` still produces clean JSON-RPC on stdout with zero log contamination. Full suite (22 packages, including the new `logx`) + `-race` green; `go vet ./...` clean throughout every step.
...
- **Moved** (`git mv`, preserving history): `internal/server` → `server/`, `internal/transport/{telegram,slack,acp}` → `transports/{telegram,slack,acp}/`. `internal/transport/tui` → `internal/tui/` (renamed, stays private — the one interactive frontend an SDK consumer never embeds programmatically, since it's a terminal UI tied to this binary). No behavior change from the move itself: `server`/`transports/*` keep importing `internal/config`/`internal/providers`/`internal/version`/`internal/logx` exactly as before — Go's `internal/` rule only blocks OTHER modules, never sibling packages in the same module, and `agent/` (already public) already imports `internal/config`/`internal/providers` directly, so this isn't a new kind of exposure.
- **Motivation**: an SDK consumer embedding the agent programmatically previously had no way to also start the HTTP/SSE server or any bot/protocol transport in-process — they'd have had to shell out to the `harness` binary. Now `harness.NewAgent(...)` + `harness.RunServer(ctx, a, ...)` / `RunTelegram` / `RunSlack` / `RunAcp` covers all of it without leaving Go.
- **New uniform "runner" shape across all four transports** — `func Run(ctx context.Context, a *agent.Agent, opts ...Option) error`, blocking until `ctx` is cancelled (or the transport's natural end, e.g. ACP's stdin closing), returning nil for that expected shutdown. Deliberately no returned `cancel` func — redundant with `ctx`, which the caller already controls.
  - **`server.Run`** (new, `server/run.go`) — packages the manual listener/goroutine/shutdown dance that used to live by hand in `internal/cli/kong_run_serve.go` into one blocking call: resolve addr (`server.WithAddr`, default `"127.0.0.1:0"`), `net.Listen`, `Serve` in a goroutine, wait for `ctx.Done()`, graceful `Close()` (sessions → agent → HTTP shutdown → unregister instance), only returning once that's fully finished. `server.WithVerbose()` enables request logging. `Server`/`NewServer`/`ServerOptions`/`Serve`/`Close` all stay exported for advanced/test use.
  - **`transports/acp.Run`** — signature changed from `Run(ctx, a, stdin, stdout)` (positional) to `Run(ctx, a, ...Option)` with `acp.WithStdin`/`acp.WithStdout` (default `os.Stdin`/`os.Stdout`).
  - **`transports/telegram.Run`** / **`transports/slack.Run`** — were already blocking runners (no shape change there), just switched from a single `Options` struct parameter to `...Option` functional options: `telegram.WithToken` (required)/`WithSessionModel`/`WithSessionThinking`/`WithAllowUnpair`; `slack.WithWorkspace`/`WithXoxC`/`WithXoxD` (required, falling back to `~/.harness/slack.json`)/`WithSessionModel`/`WithSessionThinking`.
- **Design decision — no `WithScheduler` option on any transport**: `telegram.Options`/`slack.Options` had a `Scheduler bool` field that only ever fed `agent.AgentOptions.EnableScheduler` at construction time. Since these runners now receive an ALREADY-BUILT `*agent.Agent`, that decision is made before `Run` is ever called (`agent.AgentWithScheduler()`/`AgentOptions.EnableScheduler`) — the field was dead wiring with the runner having no agent-lifecycle authority to act on it, so it's removed entirely from both `Options` structs and the facade.
- **Design decision — `Model`/`Thinking` renamed to `SessionModel`/`SessionThinking`**: unlike `Scheduler`, these ARE legitimate transport-level config — the agent itself is model-agnostic (each `Session` has its own model), and these fields override what model/thinking level sessions the TRANSPORT creates use, not the agent passed to `Run`. Renamed (both fields and their `With*` constructors) to make that scope explicit. Note: `SessionThinking` in both packages is a preexisting dead-code gap (threaded through from the CLI flag but never actually read by any wiring) — not introduced by this rename, carried over as-is from the previous `Thinking` field.
- **`harness.go` facade additions** — `RunServer`/`ServerWithAddr`/`ServerWithVerbose`; `RunTelegram`/`TelegramWithToken`/`TelegramWithSessionModel`/`TelegramWithSessionThinking`/`TelegramWithAllowUnpair`; `RunSlack`/`SlackWithWorkspace`/`SlackWithXoxC`/`SlackWithXoxD`/`SlackWithSessionModel`/`SlackWithSessionThinking`; `RunAcp`/`AcpWithStdin`/`AcpWithStdout`. All are direct aliases (`var RunX = pkg.Run`) — no reimplementation, same pattern as `NewAgent`/`AgentWith*`.
- **`internal/cli` updated to the new call shapes**: `kong_run_serve.go` collapses to a single `server.Run(ctx, a, server.WithAddr(c.Addr), server.WithVerbose())` call; `kong_run_telegram.go`/`kong_run_slack.go`/`kong_run_acp.go` build the same `Option` slices the facade would, with `c.Scheduler` continuing to flow into `newInteractiveAgent(c.Scheduler, ...)` (agent construction) rather than any transport option.
- New tests: `server/run_test.go` (4 tests — default addr, explicit `WithAddr`/`WithVerbose`, full end-to-end serve+shutdown, bind-failure error path), `transports/acp/run_test.go` (3 tests — default stdin/stdout, both overrides), `harness_test.go` (2 new end-to-end tests — `TestRunServerAliasIsWiredEndToEnd`, `TestRunAcpAliasIsWiredEndToEnd` — driving a real HTTP request / JSON-RPC round trip through the facade's aliases alone, not just type-checking them).
- `AGENTS.md` updated throughout: Golden Rule #4, the architecture diagram/file tree, "Adding a New Command"/"Adding a New Transport" workflows, the goroutines/anti-patterns lists, and the file-size table (with refreshed line counts) all reference the new `server`/`transports/*`/`internal/tui` paths.
- Full suite + `-race` green; `go build ./...` and `go vet ./...` clean throughout every step of the migration (package move → new Run/Option APIs → facade rewrite → doc updates).
...
- **Bug #1 (model freeze) — root cause**: `agent.buildSessionTools` (called once per session, at creation/resume/fork time) built the `Subagent` tool's executor closure capturing the session's model as a plain **string**, frozen at that moment. `Session.SwitchModel` (used by `/model`, and by ACP's `session/set_config_option`) updates the session's live model — but never touched that already-built closure. Reported scenario: a session created with a rate-limited provider, switched via command to a working one — the main conversation worked fine (it reads its own live model every turn), but every subsequent `Subagent` call kept hitting the ORIGINAL, still-rate-limited model.
  - **Fix** (`agent/session.go`, `agent/agent.go`) — new `Session.CurrentModel()` getter (thread-safe, mirrors the same lock `SwitchModel` uses). `buildSessionTools` now takes a `**Session` (`sessRef`) instead of a `model` string — filled in by each of its 3 call sites (`NewSession`/`ResumeSession`/`ForkSession`) immediately after constructing the session — and the `Subagent` executor reads `(*sessRef).CurrentModel()` at EXECUTION time instead of a value captured once at construction. New test `TestCurrentModelReflectsSwitchModel` confirms `CurrentModel()` reflects a `SwitchModel` call made after the session (and its tools) were already built.
- **Bug #2 (background mode always failing), found while manually verifying bug #1's fix in a real Zed session** — every `background: true` `Subagent` call died with `"Error: context canceled"`, regardless of transport (TUI, ACP, the HTTP `/ask` or `/prompt` endpoints — verified all of them). **Root cause**: `agent/session.go`'s `drainFollowUps` gives each turn its own `context.WithCancel(parentCtx)` (so a real `/stop` can cancel it), and calls that `cancel()` **unconditionally** the instant the turn's own `promptSync` returns — with zero awareness of, or waiting for, any goroutine a tool call kicked off in the background. Since `Subagent`'s `background: true` path used to pass that exact per-turn `ctx` into its goroutine, the goroutine was killed the moment its OWN launching turn ended — which is always, immediately, since that's simply how a turn normally finishes. The intended usage (a later turn checking the result file) could never work: the goroutine never survived long enough to write anything but the cancellation error.
  - **Fix** (`agent/tools/subagent.go`) — `runSubagentBackground`'s goroutine now uses `context.Background()`, completely detached from the turn's lifecycle, instead of the `ctx` `Execute` receives. New regression test `TestSubagentBackgroundSurvivesCallerContextCancellation` mirrors `drainFollowUps`' unconditional cancellation and asserts the background executor still completes and the result file gets the real answer, not a cancellation error — verified it fails against the pre-fix code and passes against the fix.
  - **`ColleagueAsk` was NOT affected** by this second bug — confirmed and documented in `agent/tools/colleague.go`: its background path delegates over HTTP via `client.Client`, which builds its own independent `context.Background()` per request (see `Client.askCtx`) and never touches the `ctx` `Execute` is invoked with at all.
- Verified end-to-end against the compiled binary with a real long-running `harness serve` process: switched a session from a rate-limited model to a working one, launched `Subagent` with `background: true`, and confirmed the result file contained the sub-agent's real answer (not a cancellation error) once its own turn had already finished. Full suite + `-race` green.
...
- The footer's session-info line (`~/path • session-name`) now shows the
  ...
  working, e.g. `~/path • my-session (2/50)`. Appears on `turn_start`
