...
- **Requested by Gus**: publish real GitHub Releases (binaries attached, not just a bare git tag) automatically whenever `make release-push` pushes a version tag, plus a manual way to (re)build one from the Actions tab.
...
- **Requested by Gus**, after using `/goal` in practice: the dedicated `Goal` tool's fixed role prompts (`builderRolePrompt`/`testerRolePrompt`) implicitly treated the Builder/Tester sub-agents as near-blank slates, pushing the parent to over-explain project context those sub-agents already have full access to (same cwd, same tools, same skills, same read-only memory — see the "sub-agents share parent's project access" fix later in the v0.76.56 changelog entry, which was already pointing at this same tension). Once Gus refined the protocol externally into something far more rigorous (a frozen Acceptance Contract, mandatory goal-hacking detection in the Tester, a round ledger, concrete stall criteria), it became clear the protocol had grown too large and too much a "living procedure that should be editable without a recompile" to keep justifying its own purpose-built tool, hardcoded Go role prompts, and a dedicated `/goal` server command.
...
- **Reported by Gus** (screenshot): the `⚔` (crossed swords) icon communicated the adversarial MECHANISM (Builder vs. Tester), not the user-facing concept of a goal/objective being pursued. Changed to `✪` (star-in-circle) in `internal/tui/output.go` — reads as "objective", doesn't collide with any existing tool icon.
...
- **Reported by Gus**, caught in a real transcript: after a `/goal` run to gather news for several cities, the parent's own thinking showed it realizing it had the Tester's verdict ("Overall: PASS") but NOT the actual news content — the Builder's response, containing the real deliverable, was never returned by `Goal`. The parent was about to reconstruct/guess the content instead of having the genuine text — a real hallucination risk directly caused by the tool's own contract.
...
- **Requested by Gus**: the default sub-agent budget (`subagentMaxIterations`, 50) is tuned for a typical focused delegated task, but a genuinely large/multi-step one (extensive codebase exploration, many files/URLs, a long multi-phase investigation) could get cut short with no way for the calling model to ask for more room.
...
- **Reported by Gus**: a `Bash` tool call was announced (spinner, `$ Bash …`) and then the turn simply never resolved — no result, no error, nothing — until typing a new prompt ("continua") started a fresh turn. Traced through the `jade-kaiban` session's `.jsonl`: the stalled tool call left **zero trace** in the persisted history (not even an orphaned `tool_call` with no `tool_result` — the call never got far enough to be persisted at all), pointing at a connection drop mid-stream rather than a tool execution failure.
