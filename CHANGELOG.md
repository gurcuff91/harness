...
- **New public package `logx/`** — just the contract, nothing else: `Logger` interface (`Info`/`Warn`/`Error(component, event string, kv ...any)`) and `NilLogger` (the discard-everything implementation). Any SDK consumer can implement `Logger` to route harness's backend logs anywhere (stdout, a file, a remote collector), without needing (or being able to touch) harness's own concrete implementation.
- **`internal/logx` refactored** from package-level functions (`logx.Info(...)`) to `HarnessLogger{}` — a struct implementing the public `logx.Logger` contract, with a compile-time assertion (`var _ logx.Logger = HarnessLogger{}`). Same line-oriented output format as before (`LEVEL [component] event k=v`), zero behavior change — only internal/cli ever constructs `HarnessLogger{}`, passing it explicitly into every `server.Run`/`transports/telegram.Run`/`transports/slack.Run` call the real `harness` binary makes.
- **`server`**: `ServerOptions.Verbose bool` replaced with `ServerOptions.Logger logx.Logger` (defaults to `logx.NilLogger{}` when nil); new `server.WithLogger(l)` Option, default `logx.NilLogger{}`. `requestLogger` middleware is now ALWAYS registered (no longer gated by a verbose conditional) — with the default `NilLogger` it's simply a no-op call per request, which is what let the `Verbose` boolean disappear entirely in favor of "always run the middleware, inject silence when nothing should be logged."
- **`transports/telegram`, `transports/slack`**: both gain `WithLogger(l logx.Logger)` (default `logx.NilLogger{}`) and a `Transport.logger` field. ALL ~40 existing `logx.Info/Warn/Error` call sites across both packages (`telegram.go`/`commands.go`/`files.go`/`pump.go`/`upload.go`; `slack.go`/`files.go`/`pump.go`/`upload.go`) migrated to `t.logger.Info/Warn/Error` — neither package imports `internal/logx` anymore. Each transport's own in-process `server.Server` always gets `logx.NilLogger{}` explicitly (never this transport's own injected logger), so a request never gets logged twice by two different layers.
- **`transports/acp`**: deliberately has NO public `WithLogger` — it never logs anything itself (pure JSON-RPC protocol translation over stdin/stdout), so there's nothing for a caller to configure. Its in-process server always gets `logx.NilLogger{}` internally, unconditionally.
- **`internal/tui` and `internal/cli/server.go`** (the CLI's own one-shot `-p` in-process server): both now pass `server.ServerOptions{Logger: logx.NilLogger{}, ...}` explicitly, replacing the old `Verbose: false` — same silent behavior, expressed as an injected no-op logger instead of a boolean. This was the actual motivating case for the whole feature: the TUI shares stdout/stderr with the raw-mode terminal renderer, so it could never safely turn `Verbose: true` on for its own server; now that's just "don't pass a real logger," the same mechanism every other silent caller uses, instead of a boolean that only ever meant "must stay off forever here."
- **`internal/cli/kong_run_{serve,telegram,slack}.go`**: all three now pass `internal/logx.HarnessLogger{}` explicitly via the new `WithLogger` option — `harness serve`/`harness telegram`/`harness slack` log exactly as before this change. `kong_run_acp.go` and `kong_run_tui.go` are unchanged (no `WithLogger` to wire — ACP never logs, and the TUI's in-process server was already covered above).
- **`harness.go` facade**: `Logger`/`NilLogger` re-exported as aliases of `logx.Logger`/`logx.NilLogger`; `ServerWithLogger`/`TelegramWithLogger`/`SlackWithLogger` added (direct aliases, same pattern as every other `XxxWith*`); `ServerWithVerbose` removed (superseded by `ServerWithLogger`). `RunAcp`'s doc comment explicitly notes it has no `AcpWithLogger`.
- New tests: `logx/logx_test.go` (`NilLogger` satisfies the interface, no observable effect), `internal/logx/logx_test.go` (updated to call `HarnessLogger{}`'s methods — same format assertions as before), `server/proxy_test.go` (renamed `TestBroadcastSilentWithNilLogger`/`TestBroadcastLogsWithRealLogger`, now injecting a `logx.Logger` instead of flipping a bool), `server/run_test.go` (`TestRunDefaultsToNilLoggerSilently`, `TestRunUsesInjectedLoggerForRequests` — a real HTTP request through a `Run`-started server reaching an injected recording logger), `transports/telegram/logger_test.go` and `transports/slack/logger_test.go` (`WithLogger` correctly populates `Options.logger`, defaults to nil before `Run` applies `NilLogger`).
