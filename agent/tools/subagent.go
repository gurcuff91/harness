...
// result file) goes with it. "background" here means "doesn't block the
// current turn", not "survives the process" — worth remembering since the
// name mirrors ColleagueAsk's, but the durability guarantee doesn't.
//
...
func Subagent(executor SubagentExecutor) Tool {
	return Tool{
		...
		Execute: func(ctx context.Context, input json.RawMessage) (string, error) {
			...
			if req.Background {
				return runSubagentBackground(ctx, executor, req.Prompt)
			}
...
// see Subagent's doc comment for why: nothing is blocked waiting on this
// goroutine, so there is nothing to protect by cutting it off. The caller's
// ctx is still passed through (unwrapped, no WithTimeout) so a real
// cancellation — e.g. the user stopping the session — still reaches the
// sub-agent; it's specifically an ARTIFICIAL time limit that's absent here.
func runSubagentBackground(ctx context.Context, executor SubagentExecutor, prompt string) (string, error) {
